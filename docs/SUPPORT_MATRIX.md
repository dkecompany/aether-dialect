...
What the engine refuses, how to reformulate refused shapes, per-dialect behaviour notes, and the per-engine capability tables. Integrator guarantees (determinism, replay, numerics, cost caps): [Integrator guide - Guarantees](INTEGRATOR_GUIDE.md#guarantees). Trust boundaries and disclosure rules: [Security](SECURITY.md). The offline sandbox runs on **in-memory DuckDB only** - other engines apply in production connections.

**Reading order:** [README](../README.md) → [Getting started](GETTING_STARTED.md) → [User guide](USER_GUIDE.md) → [Integrator guide](INTEGRATOR_GUIDE.md) → [Sandbox guide](SANDBOX.md) → [API reference](API_REFERENCE.md) → [Troubleshooting](TROUBLESHOOTING.md) → [How it works](HOW_IT_WORKS.md) → [Security](SECURITY.md) → this document.
...
| [Refused constructs](#refused-constructs) | Refusals and reformulation guidance |
...
Each refusal records a concrete reason. Reformulation is suggested only where one exists. Enforcement of the analytical subset is in [Security - Why SQL injection is not possible](SECURITY.md#4-why-sql-injection-is-not-possible).
...
| Window functions outside the whitelist | Outside the analytical IR |
| JSON path operators | Escape the typed column model the schema graph is built on |
| Custom aggregates | Outside the analytical IR |
| `PIVOT` | Output column set depends on data values, so the shape is not knowable at validation |
| `GROUPING SETS`, `ROLLUP`, `CUBE` | Subtotals change result grain mid-set; outside the analytical IR |
...
| Query-log source (for example `performance_schema`, `pg_stat_statements`, `svl_qlog`, `QUERY_HISTORY`, `JOBS`, `V$SQL`) | Catalog view or table used for query-log warmup when available. |
| `n/a` | The capability does not apply to that engine. For **Federation coordinator**, `n/a` means the engine does not *host* the coordinator (only in-process DuckDB does); the engine may still be a federation *member*. |
| `inject` / `partition inject` / `sortkey/distkey inject` / `Delta (inject)` / `cluster-key inject` / `partition inject + guard` | Partition or cluster pruning appends predicates from reflected metadata when your question's filters match. |
...
| `full (InnoDB)` / `full` / `full (\`sys.*\`)` / `Unity Catalog` / `PRAGMA` / `SQLAlchemy` | Foreign-key reflection source and completeness. |
| `partial` | Catalog FK reflection exists but is incomplete enough that operators should supply a DDL file or structure document when edges are missing (Redshift). |
...
| `->` chains in **Result reader backends** | Preference order of row-fetch backends (first available wins). |
...
- Session results: single-engine `SessionStep.sql` is a dialect string; federated analytical turns return `step.sql` as a member `source_id` → SQL mapping. Metadata questions finish with `answer` set. After accept, re-run with `execute_template(step.template_id, params)`; use `export_structure(space=...)` and `export_knowledge(space=...)` for structured catalog and knowledge read-back.

## Dialect-specific notes

Registered engines (canonical order): `sqlite`, `duckdb`, `csv`, `mysql`, `mariadb`, `sqlserver`, `postgresql`, `redshift`, `databricks`, `snowflake`, `bigquery`, `oracle`. There is **no** separate `excel` engine - Excel workbooks (`.xlsx`, not `.xls`) are loaded by the **`csv`** file engine into in-memory DuckDB.
...
- **Upload inspection** — **Inspect first, then construct.** Four severities (**Advisory**, **Review**, **Blocking**, **Fatal**): [User guide — CSV and Excel uploads](USER_GUIDE.md#csv-and-excel-uploads). Call `inspect_tabular_upload` before `AetherEngine(...)`; pass accepted `source_selections` on construction. Construction without selections when **Review** issues remain raises `ConfigError` with the report attached.
...
- AST validation uses `sqlglot` with the Oracle dialect.
- EXPLAIN uses `EXPLAIN PLAN FOR` and reads root cardinality from `PLAN_TABLE`.
- Query-log warmup reads `V$SQL` when the session can select from that view; otherwise the source reports unavailable.
- Auth modes are password (default), wallet (`wallet_location` / `config_dir`), and token (`access_token` via python-oracledb). Connections use SQLAlchemy `oracle+oracledb` in thin mode by default.
- Reflection uses `ALL_*` dictionary views, including identity columns and partition key metadata when present.
- `LIMIT` / `OFFSET` transpile to `OFFSET … FETCH NEXT …`.
- `array_contains` predicates are not supported; `JSON_TABLE` render paths apply to array unnest only.
- Profiling large tables uses a deterministic ordered-limit sample; statement timeouts at **execute** use python-oracledb `call_timeout`, not a session `SET`. Schema profiling does not apply a portable `profile_statement_timeout_sql` hook on Oracle, so `profile_timeout_ms` does not bound profiling queries. Supplied DDL files are parsed with **sqlglot** (Oracle dialect).
- Partition pruning injects predicates when your question's filters match reflected partition metadata.
...
- **No foreign-key reflection.** Multi-table join planning requires a supplied DDL file with join hints or a structure document when profiling cannot infer edges.
...
- Foreign-key reflection loads from `information_schema` and `svv_foreign_keys` but is **partial**; supply a DDL file or structure document when join edges are missing from the catalog.
...
| Capability | SQLite | DuckDB | CSV | MySQL | MariaDB | SQL Server | PostgreSQL | Redshift | Databricks | Snowflake | BigQuery | Oracle |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| EXPLAIN form | `EXPLAIN QUERY PLAN` | plain `EXPLAIN` (text) | plain `EXPLAIN` (text) | `EXPLAIN FORMAT=JSON` | `EXPLAIN FORMAT=JSON` | `SHOWPLAN_ALL` + `SHOWPLAN_XML` | `EXPLAIN (FORMAT JSON)` | plain `EXPLAIN` | `EXPLAIN COST` | `EXPLAIN USING JSON` | dry-run job | `EXPLAIN PLAN` + `PLAN_TABLE` |
| Query-log source | n/a | n/a | n/a | `performance_schema` | `performance_schema` | Query Store -> `dm_exec_query_stats` | `pg_stat_statements` | `svl_qlog` | `system.query.history` | `QUERY_HISTORY` | `JOBS` | `V$SQL` |
| SQL-file parse grammar | sqlglot (sqlite) | sqlglot (duckdb) | n/a | sqlglot (mysql) | sqlglot (mysql) | sqlglot (tsql) | pglast | sqlglot (redshift) | sqlglot (databricks) | sqlglot (snowflake) | sqlglot (bigquery) | sqlglot (oracle) |
| Partition / cluster pruning | no-op | inject | inject | partition inject | partition inject | partition inject | partition inject | sortkey/distkey inject | Delta (inject) | cluster-key inject | partition inject + guard | partition inject |
| FK catalog reflection | PRAGMA | SQLAlchemy | n/a | full (InnoDB) | full (InnoDB) | full (`sys.*`) | full | partial | Unity Catalog | full | **none** | full (`ALL_*`) |
| Result reader backends | SQLAlchemy | SQLAlchemy | SQLAlchemy | connector -> SQLAlchemy | connector -> SQLAlchemy | SQLAlchemy | SQLAlchemy | connector -> SQLAlchemy | connector -> SQLAlchemy -> Spark | connector (Arrow) -> SQLAlchemy -> Snowpark | `bq_storage` -> SQLAlchemy | SQLAlchemy |
| Federation coordinator | n/a | DuckDB in-process | member only | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |
| Cross-source decomposition | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue | per-source + glue |

---

**See also:** [Getting started](GETTING_STARTED.md) | [User guide](USER_GUIDE.md) | [Integrator guide](INTEGRATOR_GUIDE.md) | [Sandbox guide](SANDBOX.md) | [API reference](API_REFERENCE.md) | [Troubleshooting](TROUBLESHOOTING.md) | [How it works](HOW_IT_WORKS.md) | [Security](SECURITY.md) | [README](../README.md)
