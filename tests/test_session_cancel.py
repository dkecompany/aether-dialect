...
def test_cancel_delegates_federation_context() -> None:
    ...
    session.active_federation_execution_context = ctx
...
def test_cancel_sets_turn_event_when_busy() -> None:
    ...
    session._session_busy = True
    session._turn_cancel_event = threading.Event()
    session.active_federation_execution_context = None
...
@pytest.mark.fast
def test_cancel_delegates_to_cancel() -> None:
    session = PipelineSession(MagicMock())
    session._session_busy = True
    session._turn_cancel_event = threading.Event()
...
def test_session_turn_cancelled_raises_in_interactive_run_once() -> None:
    with patch("aetherdialect._main_spaces.session_turn_cancelled", return_value=True):
...
def test_cancel_while_suspended_frees_session_for_ask() -> None:
    ...
    assert cancel_step.error is not None
    assert cancel_step.error.code.value == "cancelled"
    ...
    with patch("aetherdialect._main_init.MainInitOps.interactive_run_once", side_effect=_complete_turn):
