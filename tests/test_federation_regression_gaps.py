...
@pytest.mark.fast
def test_federation_sandbox_seed_files_present_and_parse() -> None:
    """Bundled federation seeds under scripts/data must load without parse errors."""
    declaration_path = _DATA_ROOT / "federation_declaration.json"
    if not declaration_path.is_file():
        pytest.skip("federation sandbox seeds are not present under scripts/data")
...
@pytest.mark.fast
def test_federation_sandbox_offline_smoke_when_seeds_exist() -> None:
    """When partition seeds exist, federation declaration composes a multi-source graph."""
    declaration_path = _DATA_ROOT / "federation_declaration.json"
    if not declaration_path.is_file():
        pytest.skip("federation sandbox seeds are not present under scripts/data")
    ...
    with llm_usage_session_scope():
        with llm_usage_build_scope():
            with (
                ...
                patch("aetherdialect._pipeline.get_join_choice_from_llm", side_effect=_fake_join),
            ):
