...
class TestIntentKey:
    ...
    def test_bool_op_same_key_when_leaves_match(self):
        """intent_key hashes leaf predicates; bool_op does not affect the key."""
        skeleton = {
            "tables": ["t"],
            "grain": "row_level",
        ...
        i_and = RuntimeIntent(
            **skeleton,
            where=_from_flat_where_dicts(legacy_and),
        )
        i_or = RuntimeIntent(
            **skeleton,
            where=_from_flat_where_dicts(legacy_or),
        )
        assert intent_key(i_and) == intent_key(i_or)

    def test_where_group_same_key_when_leaves_match(self):
        """intent_key hashes leaf predicates; where_group does not affect the key."""
        skeleton = {
            "tables": ["t"],
            "grain": "row_level",
            "select_cols": [SelectCol(expr=NormalizedExpr.from_column("t.col"))],
        ...
        i_grouped = RuntimeIntent(
            **skeleton,
            where=_from_flat_where_dicts(legacy_grouped),
        )
        i_split = RuntimeIntent(
            **skeleton,
            where=_from_flat_where_dicts(legacy_split),
        )
    ...
    def test_different_having_bool_op_same_key_when_leaves_match(self):
        """intent_key hashes leaf having predicates; bool_op does not affect the key."""
        skeleton = {
            "tables": ["t"],
            "grain": "grouped",
            "select_cols": [],
        ...
        i_and = RuntimeIntent(
            **skeleton,
            having=_from_flat_having_dicts(legacy_and),
        )
        i_or = RuntimeIntent(
            **skeleton,
            having=_from_flat_having_dicts(legacy_or),
        )
        assert intent_key(i_and) == intent_key(i_or)

    def test_different_having_where_group_same_key_when_leaves_match(self):
        """intent_key hashes leaf having predicates; where_group does not affect the key."""
        skeleton = {
            "tables": ["t"],
            "grain": "grouped",
            "select_cols": [],
        ...
        i_grouped = RuntimeIntent(
            **skeleton,
            having=_from_flat_having_dicts(legacy_grouped),
        )
        i_split = RuntimeIntent(
            **skeleton,
            having=_from_flat_having_dicts(legacy_split),
        )
        assert intent_key(i_grouped) == intent_key(i_split)
...
class TestNormalizeFilters:
    ...
    def test_where_group_rows_via_predicate_group(self):
    ...
    def test_sorts_by_signature_not_where_group(self):
        """normalize_filters sorts by signature_key, not where_group."""
...
class TestNormalizeHavingConditions:
    ...
    def test_bool_op_rows_via_predicate_group(self):
        ...
        group = _from_flat_having_dicts(legacy)
        assert group is not None
        assert group.op == "or"
    ...
    def test_where_group_rows_via_predicate_group(self):
        ...
        assert result[0].left_expr.add_groups[0].multiply[0].column_ref == "t.id"
        assert result[1].left_expr.add_groups[0].agg_func == "sum"
        assert result[1].left_expr.add_groups[0].multiply[0].column_ref == "t.amount"
        assert not hasattr(result[0], "where_group")

    def test_sorts_by_signature_not_where_group(self):
        """normalize_having_conditions sorts by signature_key, not where_group."""
...
class TestNormalizeCteStepsForKey:
    ...
    def test_filter_signatures_in_key_from_leaves(self):
        """normalize_cte_steps_for_key includes leaf signature_key, not bool_op."""
        fp = WhereParam(
    ...
    def test_where_group_not_in_key(self):
        """normalize_cte_steps_for_key omits where_group from filter key entries."""
...
class TestNormalizeFiltersEdgeCases:
    ...
    def test_dict_ignores_bool_op(self):
        """normalize_filters ignores bool_op dict keys; use nested PredicateGroup trees."""
    ...
    def test_dict_normalizes_without_bool_fields(self):
        """normalize_filters ignores bool_op/where_group dict keys."""
...
class TestNormalizeHavingEdgeCases:
    ...
    def test_dict_normalizes_without_bool_fields(self):
        """normalize_having_conditions ignores bool_op/where_group dict keys."""
        d = {
            "aggregation": "COUNT(t.id)",
            "op": ">",
            "value_type": "integer",
...
class TestNormalizeCteStepsEdgeCases:
    ...
    def test_dict_with_filters(self):
        """normalize_cte_steps converts dict with nested where predicate groups."""
        steps = [
            {
                "cte_name": "cte1",
                "tables": ["t1"],
    ...
    def test_dict_with_having(self):
        """normalize_cte_steps converts dict with nested having predicate groups."""
        steps = [
            {
                "cte_name": "cte1",
                "tables": ["t1"],
...
class TestValidateQuestion:
    ...
    def test_valid_allowed_returns_true_and_corrected(self, mock_llm_json):
        """validate_question maps allowed to analytical and returns corrected text."""
    ...
    def test_restricted_wins_when_valid_flag_no(self, mock_llm_json):
        """Restricted is reported even when valid_database_question is no."""
        mock_llm_json.return_value = {
            "valid_database_question": "no",
            "query_type": "restricted",
    ...
    def test_cte_analytical_may_be_allowed_not_restricted(self, mock_llm_json):
        """Analytical questions that mention CTEs are classified analytical when the model agrees."""
        mock_llm_json.return_value = {
            "valid_database_question": "yes",
            "query_type": "allowed",
...
class TestNormalizationGuard:
    """Tests for :func:`aetherdialect._utils_intent._enforce_normalization_guard`."""

    def test_aggregation_prefix_and_particles(self):
...
class TestGenerateQuestion:
    ...
    @patch("aetherdialect._utils_intent.pick_question_style", return_value="How many")
    @patch("aetherdialect._utils_intent.LLMProvider.json")
    ...
    def test_returns_none_when_llm_returns_empty(self, mock_llm_json, minimal_schema):
        """generate_question raises when response lacks a usable question key."""
        mock_llm_json.return_value = {}
        with pytest.raises(ValueError, match="missing 'question' key"):
            generate_question(
                tables=["orders"],
    ...
    def test_returns_none_when_question_key_missing(self, mock_llm_json, minimal_schema):
        """generate_question raises when response has no 'question' key."""
...
class TestGenerateQuestionFromSql:
    ...
    @patch("aetherdialect._utils_intent.generate_warmup_paraphrases_by_style", return_value=None)
    @patch("aetherdialect._utils_intent.LLMProvider.json")
    ...
    @patch("aetherdialect._utils_intent.LLMProvider.json")
    def test_realistic_uses_question_field_when_questions_empty(self, mock_llm, empty_schema):
        """Empty questions array uses the question string."""
        mock_llm.return_value = {
            "is_realistic": True,
            "questions": [],
    ...
    @patch("aetherdialect._utils_intent.LLMProvider.json", side_effect=RuntimeError("boom"))
...
class TestGenerateQuestionPhrasing:
    ...
    @patch("aetherdialect._utils_intent.pick_question_style", return_value="Prefix{ignored}")
    @patch("aetherdialect._utils_intent.LLMProvider.json")
    ...
    @patch("aetherdialect._utils_intent.LLMProvider.json", side_effect=ValueError("bad json"))
    @patch("aetherdialect._utils_intent.pick_question_style", return_value="What are")
