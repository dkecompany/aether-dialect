"""Fast regressions for join kinds, predicates, SQL hygiene, coordinator correctness, and scope."""
...
def test_federation_turn_state_isolated_between_owners() -> None:
    ...
    session_a = PipelineSession(owner_a)
    session_b = PipelineSession(owner_b)
...
def test_session_reset_does_not_clear_sibling_session_pending_plan() -> None:
    ...
    session_a._pending_federation_plan_template = tmpl_a
    session_b._pending_federation_plan_template = tmpl_b
    session_b._reset_after_turn()
    assert session_a._pending_federation_plan_template is tmpl_a
    assert session_b._pending_federation_plan_template is None
...
@pytest.mark.fast
def test_federation_space_deny_objects_reach_planning() -> None:
    fed = build_two_member_federation()
    intent = RuntimeIntent(
        tables=[fed.left_table],
        grain="many",
    ...
    space = SpaceContext(deny_objects=frozenset({fed.left_table}))
    plan = plan_federated_intent(intent, fed.composite, fed.manifest, space=space)
    assert plan.ineligible_reason == "no tables referenced"


@pytest.mark.fast
def test_space_deny_column_blocks_federated_projection() -> None:
    fed = build_two_member_federation()
    intent = RuntimeIntent(
        tables=[fed.left_table],
        grain="many",
    ...
    space = SpaceContext(deny_columns=frozenset({f"{fed.left_table}.id"}))
    plan = plan_federated_intent(intent, fed.composite, fed.manifest, space=space)
