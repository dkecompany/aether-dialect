"""Migration tier behaviors: additive tier, diff visibility, rename confidence, atomic map apply, auxiliary artifacts."""
...
class TestAdditiveTiers:
    ...
    def test_additive_print_header_reports_additions(self) -> None:
        ...
        lines: list[str] = []
        MainExecutionOps.print_migration_applied(report, lines.append)
        assert lines
...
class TestAtomicMapApply:
    def test_rollback_on_mid_sequence_failure(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("aetherdialect._templates_ops.migrate_sidecar_for_diff", _boom)
    ...
    def test_rollback_restores_sidecar_and_aetherspace(self, tmp_path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("aetherdialect._schema_finalize.migrate_sidecar_for_diff", _touch_sidecar)
        monkeypatch.setattr("aetherdialect._templates_ops.TemplateOps.apply_structural_migration_from_map", _boom)
        with pytest.raises(RuntimeError, match="boom"):
