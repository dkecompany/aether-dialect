...
import aetherdialect._main_execution as main_execution_module
...
class TestLoadConfigFile:
    ...
    def test_full_coverage_flatten(self, tmp_path) -> None:
        ...
        got, claimed = main_execution_module._load_config_file(str(path))
        expected = {
            "OPENAI_API_KEY": "oak",
...
def test_merge_toml_diagnostic_when_toml_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    merged, keys = main_execution_module._merge_configuration_environment({"POSTGRES_HOST": "toml_host"})
...
class TestPrepareSchemaContextForInit:
    ...
    def test_reuses_cached_sql_when_missing_in_explicit_context(self, tmp_path) -> None:
        ...
        write_schema_context_cache(engine_dir, SchemaContext(sql_file=str(sql)))
        logs: list[str] = []
        out = _prepare_schema_context_for_init(SchemaContext(), engine_dir, logs.append)
...
class TestFindLatestSeedWarmupSummary:
    """Tests for :func:`aetherdialect._main_execution.find_latest_seed_warmup_summary`."""
...
class TestSelectEngineExplicit:
    ...
    def test_invalid_aether_engine_raises(self) -> None:
        with pytest.raises(ConfigError, match="Unsupported AETHERDIALECT_ENGINE"):
            _select_engine_name({"AETHERDIALECT_ENGINE": "mysql"})
...
class TestDatabricksEngineSelectionPySpark:
    ...
    def test_selects_databricks_when_pyspark_reachable(self) -> None:
        ...
        with patch.object(importlib, "import_module", side_effect=_import_module_side_effect_dbx_only):
            with patch.object(main_execution_module, "_pyspark_session_reachable", return_value=True):
                assert _select_engine_name(env) == "databricks"
    ...
    def test_raises_when_catalog_schema_only_and_no_pyspark(self) -> None:
        ...
        with patch.object(importlib, "import_module", side_effect=_import_module_side_effect_dbx_only):
            with patch.object(main_execution_module, "_pyspark_session_reachable", return_value=False):
...
class TestConfigureRuntimeFromEnvironment:
    ...
    def test_openai_ignores_optional_model_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        try:
            with patch.object(
                ...
                configure_runtime_from_environment(SchemaContext(), _snapshot_os_environ())
    ...
    def test_azure_deployment_env_overrides(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        try:
            with patch.object(
                ...
                configure_runtime_from_environment(SchemaContext(), _snapshot_os_environ())
            from aetherdialect._core_utils import _azure_deployment_for_model
    ...
    def test_both_llm_providers_configured_requires_explicit_choice(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from aetherdialect._contracts_base import ConfigError as _CE
        ...
        try:
            with patch.object(
                ...
                with pytest.raises(_CE, match="AETHERDIALECT_LLM_PROVIDER"):
                    configure_runtime_from_environment(SchemaContext(), _snapshot_os_environ())
        finally:
            for k, v in snap_eng.items():
                setattr(EngineConfig, k, v)
...
class TestSchemaContextCache:
    """Tests for write/load helpers on the persisted SchemaContext."""
    ...
    def test_write_inlines_sql_and_notes(self, tmp_path) -> None:
        ...
        out = main_execution_module.write_schema_context_cache(adir, ctx)
        assert os.path.exists(out)
        with open(out, encoding="utf-8") as f:
            data = json.load(f)
        assert data["version"] == main_execution_module.SCHEMA_CONTEXT_CACHE_VERSION
    ...
    def test_load_round_trip_materializes_files(self, tmp_path) -> None:
        ...
        main_execution_module.write_schema_context_cache(adir, ctx)
        loaded = main_execution_module.load_schema_context_cache(adir)
    ...
    def test_load_missing_returns_none(self, tmp_path) -> None:
        loaded = main_execution_module.load_schema_context_cache(str(tmp_path))
        assert loaded is None

    def test_load_bad_version_returns_none(self, tmp_path) -> None:
        adir = str(tmp_path)
        path = os.path.join(adir, main_execution_module.SCHEMA_CONTEXT_CACHE_NAME)
        with open(path, "w", encoding="utf-8") as f:
            json.dump({"version": 999}, f)
        assert main_execution_module.load_schema_context_cache(adir) is None
...
class TestPipelineSessionSuspendToStep:
    ...
    def test_sql_feedback_suspend_populates_message_prompt(self) -> None:
        from aetherdialect._config import PIPELINE_SUSPEND_ID_SQL, GenerationPath
