...
@pytest.mark.fast
def test_logistics_seed_reflects_full_partition() -> None:
...
@pytest.mark.fast
def test_crm_seed_reflects_full_partition() -> None:
...
def test_crm_customer_replica_column_drift() -> None:
    ...
    assert ", 1001," in crm
    assert ", 1005," in crm
    assert sc._crm_customer_desync_address_id(1, 1) == 1 + sc.CRM_CUSTOMER_ADDRESS_DESYNC_OFFSET
...
@pytest.mark.fast
def test_logistics_legacy_receipts_and_abbreviated_purchase_order() -> None:
...
@pytest.mark.fast
def test_orphan_delivery_rental_ids_in_logistics_seed() -> None:
    logistics = _read_seed("federation_logistics_seed.sql")
    sc = _sandbox_corpus()
...
@pytest.mark.fast
def test_subscription_retail_reskin_in_seeds_and_generator() -> None:
    catalog = _read_seed("federation_catalog_seed.sql")
    sc = _sandbox_corpus()
