...
from aetherdialect._contracts_base import ArtifactManifest, MigrationReport, MigrationTier
from aetherdialect._contracts_core import LlmJsonExhausted, RephraseHint
...
@contextmanager
def _notify_print_listener(fn):
    token = push_diagnostic_sink(lambda d: fn(d.message))
    try:
        yield
    finally:
...
class TestDebugGating:
    ...
    def test_prints_when_debug_enabled(self, capsys):
        ...
        with patch("aetherdialect._utils.diagnostic_debug_enabled", return_value=True):
            debug("hello")
    ...
    def test_silent_when_debug_disabled(self, capsys):
        ...
        with patch("aetherdialect._utils.diagnostic_debug_enabled", return_value=False):
            debug("hello")
        out = capsys.readouterr().out
        assert out == ""
...
class TestPrintQueryResult:
    ...
    def test_single_scalar(self, capsys):
        ...
        with _notify_print_listener(print):
    ...
    def test_results_heading(self, capsys):
        ...
        from aetherdialect._constants_runtime import QUERY_RESULTS_HEADER
...
class TestTelemetryCapture:
    ...
    def test_force_diagnostic_flags_restored(self, monkeypatch):
        ...
        from aetherdialect._utils import diagnostic_debug_enabled
...
class TestPipelineTrace:
    ...
    def test_skips_body_factory_when_inactive_and_no_sink(self) -> None:
        ...
        with (
            ...
            patch("aetherdialect._utils.prev_sink", None),
        ):
...
class TestAskUserChoice:
    ...
    def test_yes_short(self, capsys):
        with _notify_print_listener(print):
            with patch("aetherdialect._utils.input", return_value="y"):
    ...
    def test_no_short_silent_no(self, capsys):
        with _notify_print_listener(print):
            with patch("aetherdialect._utils.input", return_value="n"):
    ...
    def test_eof_returns_none(self, capsys):
        with _notify_print_listener(print):
            with patch("aetherdialect._utils.input", side_effect=EOFError()):
    ...
    def test_keyboard_interrupt_returns_none(self, capsys):
        with patch("aetherdialect._utils.input", side_effect=KeyboardInterrupt()):
            assert ask_user_choice("OK?", ["y", "n"]) is None

    def test_empty_input_invalid(self, capsys):
        with _notify_print_listener(print):
            with patch("aetherdialect._utils.input", return_value=""):
                assert ask_user_choice("OK?", ["y", "n"]) is None
        assert "Invalid input." in capsys.readouterr().out

    def test_garbage_invalid(self, capsys):
        with patch("aetherdialect._utils.input", return_value="maybe"):
            assert ask_user_choice("OK?", ["y", "n"]) is None
...
class TestInteractiveYesNo:
    ...
    def test_falls_back_to_ask_user_choice(self, capsys):
        with patch("aetherdialect._utils.input", return_value="yes"):
...
class TestArtifactManifest:
    ...
    def test_classify_schema_diff_implies_remap_when_rename_plan_missing(self) -> None:
        ...
        with patch("aetherdialect._schema_graph.try_rename_migration_plan", return_value=None):
...
class TestTelemetryCapturesPipelineTrace:
    ...
    def test_pipeline_trace_structured_sink_without_debug(self):
        events: list[tuple[str, str]] = []

        def _capture(heading: str, body: str) -> None:
            events.append((heading, body))

        with pipeline_trace_sink(_capture):
            with patch("aetherdialect._utils.diagnostic_debug_enabled", return_value=False):
                pipeline_trace("timing.phase", "elapsed=12ms")
        assert events == [("timing.phase", "elapsed=12ms")]
            ...
            with patch("aetherdialect._llm_provider.LLMProvider._provider_is_configured", return_value=True):
                with patch("aetherdialect._llm_provider.LLMProvider._build_client", return_value=client):
                    with patch("aetherdialect._utils.debug"):
                        with patch("aetherdialect._utils.pipeline_trace"):
                            with patch("aetherdialect._utils.debug"):
                                with patch("aetherdialect._utils.time.sleep"):
...
class TestWriteGzipJsonAtomicFailure:
    ...
    def test_unlinks_temp_when_replace_fails(self, tmp_path):
        ...
        with patch(
            "aetherdialect._utils.os.replace",
...
class TestAskUserChoiceMore:
    ...
    def test_no_prints_terminated_when_not_silent(self, capsys):
        with _notify_print_listener(print):
            with patch("aetherdialect._utils.input", return_value="no"):
...
class TestPrintRephraseHint:
    ...
    def test_restricted_question_hint(self, capsys):
        ...
        assert "operation" in out and "not supported" in out
...
class TestPrompt:
    ...
    def test_strips_whitespace(self):
        with patch("aetherdialect._utils.input", return_value="  hi  "):
            assert prompt("Q? ") == "hi"
