"""SQL construction: bound parameters, identifier quoting, DISTINCT ON wrapping, AST predicate chains, and dialect parse checks."""
...
def test_sqlalchemy_backend_passes_bind_map_to_execute() -> None:
    ...
    backend = SqlAlchemyResultBackend(engine, dialect_name="duckdb")
...
def _where_leaf(pred: WhereParam) -> str:
    quoted_column = pred.left_expr.primary_column.replace(".", '"."')
    return f'"{quoted_column}" = :p1'
...
def test_sqlglot_quote_identifier_helper_exists_and_matches_generator() -> None:
    ...
    for dialect_name in ("postgres", "duckdb", "mysql", "tsql"):
        for ident in ("order", "plain_table", 'has"quote'):
            assert Dialect.sqlglot_quote_identifier(ident, dialect_name) == _expected_sqlglot_quote(ident, dialect_name)
...
def test_federation_quote_ident_quotes_reserved_words_via_sqlglot() -> None:
    ...
    assert Dialect.sqlglot_quote_identifier("plain_table") == Dialect.sqlglot_quote_identifier("plain_table", "duckdb")
