...
def test_in_memory_via_static_pool_execution_engine() -> None:
    connection = sqlite3.connect(":memory:", check_same_thread=False)
    _seed_memory_table(connection)
    execution_engine = SQLiteDialect.create_sqlite_sqlalchemy_engine(connection)
    dialect = DialectRegistry.get("sqlite", SQLiteRuntimeConfig, sqlalchemy_engine=execution_engine)
