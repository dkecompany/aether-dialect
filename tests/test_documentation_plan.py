"""Documentation structure and refusal tables required by the release plan."""
...
@pytest.mark.fast
def test_changelog_exists_as_release_notes() -> None:
    path = _REPO / "CHANGELOG.md"
    assert path.is_file()
    text = path.read_text(encoding="utf-8")
    assert "0.2.0" in text
    assert "declaration" in text.lower()
...
# --- S9: one vocabulary across documents ---
...
@pytest.mark.fast
def test_s9_upload_guides_share_canonical_phrase() -> None:
...
@pytest.mark.fast
def test_s9_upload_construction_failures_raise_config_error() -> None:
    for path in _UPLOAD_GUIDES:
        text = path.read_text(encoding="utf-8")
...
@pytest.mark.fast
def test_s9_security_upload_links_use_canonical_anchor() -> None:
...
@pytest.mark.fast
def test_s9_role_vocabulary_engine_role_and_session_mode() -> None:
...
@pytest.mark.fast
def test_s9_federation_declaration_canonical_filename() -> None:
...
@pytest.mark.fast
def test_s9_user_guide_federation_reentry_links_not_internal_exports() -> None:
...
@pytest.mark.fast
def test_s9_user_guide_aetherfederation_uses_declaration_not_manifest() -> None:
    text = (_DOCS / "USER_GUIDE.md").read_text(encoding="utf-8")
...
@pytest.mark.fast
def test_s9_troubleshooting_refusal_codes_use_refusal_wording() -> None:
    text = (_DOCS / "TROUBLESHOOTING.md").read_text(encoding="utf-8")
    for code in _REFUSAL_DIAGNOSTIC_CODES:
        row = re.search(rf"\| `{code}` \|([^|]+)\|", text)
        assert row is not None, f"missing troubleshooting row for {code}"
        assert "refusal" in row.group(1).lower(), f"{code} row must describe a refusal, not an error"


@pytest.mark.fast
def test_s9_support_matrix_links_user_guide_for_upload_detail() -> None:
    text = (_DOCS / "SUPPORT_MATRIX.md").read_text(encoding="utf-8")
