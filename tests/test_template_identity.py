...
def test_remap_concrete_intent_rewrites_select_and_where_columns() -> None:
    ...
    remapped = TemplateOps._remap_concrete_intent(concrete, {"child": "kid"}, {"child": {"score": "points"}})
    ...
    refs = TemplateRefs.template_schema_refs(
        replace(
            _join_template(),
...
@pytest.mark.fast
def test_remap_concrete_intent_rewrites_having_columns() -> None:
    having = PredicateGroup.from_list(
        [HavingParam(left_expr=NormalizedExpr.from_column("child.score"), op=">", right_expr=None, param_key="p1")]
    )
    concrete = ConcreteIntent(
        intent_id="id",
        tables=["child"],
    ...
    remapped = TemplateOps._remap_concrete_intent(concrete, {"child": "kid"}, {"child": {"score": "points"}})
    assert (remapped.having.leaves() if remapped.having else [])[0].left_expr.column_ref == "kid.points"
...
def test_join_preset_scope_from_concrete_pins_main_and_cte() -> None:
    ...
    concrete = replace(concrete, cte_steps=[cte.to_concrete()])
...
def test_join_path_key_differs_for_candidate_id_and_cte_emission() -> None:
    ...
    with_join = replace(base, cte_steps=[cte_join.to_concrete()])
    with_semi = replace(base, cte_steps=[cte_semi.to_concrete()])
