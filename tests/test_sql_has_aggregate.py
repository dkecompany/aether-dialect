"""sql_has_aggregate must agree with AGGREGATE_FUNCTION_NAMES / IR classification."""
...
from aetherdialect._dialect import sql_has_aggregate
...
def test_sql_has_aggregate_matches_authoritative_names(sql_fn: str) -> None:
    ...
    assert sql_has_aggregate(sql, sqlglot_dialect="postgres") is True
...
def test_sql_has_aggregate_rejects_non_aggregates() -> None:
    assert sql_has_aggregate("SELECT x FROM t", sqlglot_dialect="postgres") is False
    assert sql_has_aggregate("SELECT LOWER(x) FROM t", sqlglot_dialect="postgres") is False
    assert sql_has_aggregate("SELECT COALESCE(x, 0) FROM t", sqlglot_dialect="postgres") is False
