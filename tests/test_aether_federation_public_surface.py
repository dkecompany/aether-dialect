...
_MANIFEST_FILE = str(_TMP / "aether_fed_public_declaration.json")
...
def _minimal_member(*, connection: str = "conn") -> MagicMock:
    ...
    member._runtime_config = RuntimeConfig(
        engine="duckdb",
        artifacts_dir=str(_TMP / f"aether_fed_member_{connection}"),
    ...
    member._artifacts_dir = str(_TMP / f"aether_fed_member_{connection}")
...
def _init_bundle(manifest: FederationManifest, composite: SchemaGraph) -> AetherFederationInitResult:
    ...
    return AetherFederationInitResult(
        runtime_config=RuntimeConfig(
            engine="duckdb",
            artifacts_dir=str(_TMP / "aether_fed"),
            engine_context=MagicMock(),
        ...
        federation_storage_dir=str(_TMP / "aether_fed" / "fed_public"),
        federation_source_runtimes={
            "a": MagicMock(
                ...
                artifacts_dir=str(_TMP / "aether_fed_member_a"),
            ),
            "b": MagicMock(
                source_id="b",
                dialect=MagicMock(),
...
def test_dispose_skips_borrowed_member_connections() -> None:
    ...
    MainExecutionOps.dispose_federation_source_runtimes({"a": runtime}, member_engines={"conn": member})
