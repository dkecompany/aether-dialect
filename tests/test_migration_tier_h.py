...
class TestH1AdditiveTier:
    ...
    def test_additive_policy_preserves_templates(self, tmp_path) -> None:
        ...
        store = _load_partitioned_view_unlocked(template_store_dir_for_space(str(tmp_path), "master"))
...
class TestH2DiffVisibility:
    ...
    def test_fk_change_tombstones_table(self, tmp_path) -> None:
        ...
        deleted = surgical_invalidate_templates_by_diff(str(tmp_path), new, diff)
        assert deleted == 1

    def test_profiling_drift_below_overlap_is_soft_refresh(self) -> None:
        manifest = ArtifactManifest(
            artifact_format_version=ARTIFACT_FORMAT_VERSION,
...
class TestH7AuxiliaryArtifacts:
    ...
    def test_apply_migration_soft_refresh_clears_qsim(self, tmp_path) -> None:
        ...
        apply_migration_policy(str(tmp_path), schema, schema_diff=diff)
