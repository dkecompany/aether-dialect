...
from aetherdialect._intent_process import _trace_intent_after_deterministic_step


@pytest.mark.fast
def test_headings_come_from_the_fixed_set() -> None:
    heading_map = getattr(_constants, "DETERMINISTIC_REPAIR_TRACE_HEADINGS", None)
    ...
    empty = RuntimeIntent()
    with (
        patch("aetherdialect._intent_process.pipeline_trace", _capture),
        patch("aetherdialect._intent_process.diagnostic_debug_enabled", return_value=True),
        patch("aetherdialect._intent_process.diagnostic_pipeline_trace_full_enabled", return_value=True),
    ):
