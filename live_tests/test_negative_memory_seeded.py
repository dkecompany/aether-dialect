"""
Seeded tests for question-level feedback (validation + intent rejections).

Each group seeds the artefact it owns and asserts only the observable output of that subsystem:
persisted rows under ``question_feedback``, prompt collection via
:func:`aetherdialect._templates.collect_question_feedback_for_prompt`, and penalty mapping via
:func:`aetherdialect._templates.compute_question_feedback_penalty`.
"""
...
from aetherdialect._templates import (
    ...
    compute_intent_structural_signature,
...
def test_rejected_template_seeds_avoid_example(schema, schema_terms, t2s) -> None:
    ...
    with isolated_runner(schema, schema_terms, t2s, label="nm_avoid") as runner:
        ...
        ish, _ = compute_intent_structural_signature(intent_avoid)
        rt = seed_rejected(
            runner,
...
def test_negative_memory_penalty_applies_to_matching_template(schema, schema_terms, t2s) -> None:
    """
    Seeded validation rows increase ``compute_question_feedback_penalty`` for the template's question.

    One append must add at least ``PEN_BY_THREE_SOURCE_UNIT`` and stay under ``PENALTY_CAP``.
    """
    ...
    with isolated_runner(schema, schema_terms, t2s, label="nm_penalty") as runner:
        ...
        assert abs(penalty - expected) < 1e-6, (
            f"[NM-PENALTY] expected {expected}; got {penalty} (unit={PolicyConfig.PEN_BY_THREE_SOURCE_UNIT})"
        )
...
def test_negative_memory_penalty_caps_at_policy_limit(schema, schema_terms, t2s) -> None:
    ...
    with isolated_runner(schema, schema_terms, t2s, label="nm_penalty_cap") as runner:
        ...
        assert pen <= PolicyConfig.PENALTY_CAP + 1e-9, (
            f"[NM-PENALTY-CAP] penalty exceeds cap: {pen} > {PolicyConfig.PENALTY_CAP}"
        )
