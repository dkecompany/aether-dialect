...
from ._knowledge_join import notes_ratchet_multiplies_table
from ._schema_graph import fk_infer_value_types_compatible
from ._schema_reflect import column_user_display_name, tables_referenced_in_view_definition
from ._utils import (
...
def _validate_scalar_func_valid(
    ...
    if func_lower not in VALID_SCALAR_FUNCTIONS:
        ...
        debug(f"[validation_shape.validate_scalar_func_valid] invalid scalar '{scalar_func}' in {location}")
        return issues
    if col_meta is not None and func_lower in SCALAR_FUNCTIONS_TEMPORAL:
        ...
        if col_value_type and col_value_type != "date":
            ...
            debug(
                f"[validation_shape.validate_scalar_func_valid] temporal '{scalar_func}' on "
    ...
    if _is_extract_epoch(expr.scalar_func, expr.scalar_func_args or []):
        issues.append(
            IntentIssue.make(
                ...
                severity="error",
                message=(
                    "EXTRACT(EPOCH FROM ...) is not supported. Use date column subtraction or other supported date functions."
                ),
...
def _validate_agg_func_valid(agg_func: str | None, context: str, location: str) -> list[IntentIssue]:
    ...
    if func_lower not in VALID_AGGREGATION_FUNCTIONS:
        ...
        debug(f"[validation_shape.validate_agg_func_valid] invalid agg '{agg_func}' in {location}")
    return issues
...
def validate_select_cols_schema(
    ...
    *,
    ...
    debug(f"[validation_shape.validate_select_cols_schema] {len(issues)} issues in {context}")
    return issues
...
def validate_order_by_cols_schema(
    ...
    debug(f"[validation_shape.validate_order_by_cols_schema] {len(issues)} issues in {context}")
    return issues
...
def validate_group_by_cols_schema(
    ...
    debug(f"[validation_shape.validate_group_by_cols_schema] {len(issues)} issues in {context}")
    return issues
...
def validate_filters_schema(
    ...
    *,
    ...
    debug(f"[validation_shape.validate_filters_schema] {len(issues)} issues in {context}")
    return issues
...
def _validate_having_agg(
    ...
    if table_name in schema.tables:
        ...
        if not table_col_meta:
            ...
            allowed_types: tuple[str, ...] = AGGREGATION_ALLOWED_COLUMN_TYPES.get(func, ())
...
def validate_having_schema(
    ...
    *,
    ...
    debug(f"[validation_shape.validate_having_schema] {len(issues)} issues in {context}")
    return issues
...
def validate_where_ops_per_column(
    ...
    debug(f"[validation_shape.validate_where_ops_per_column] {len(issues)} issues in {context}")
    return issues
...
def validate_having_ops_per_column(
    ...
    if issues:
        debug(f"[validation_shape.validate_having_ops_per_column] {len(issues)} issues in {context}")
    return issues
...
def validate_date_window_units(
    ...
    *,
    ...
    if issues:
        debug(f"[validation_shape.validate_date_window_units] {len(issues)} invalid units")
    return issues
...
def validate_date_diff_units(
    ...
    *,
    ...
    if issues:
        debug(f"[validation_shape.validate_date_diff_units] {len(issues)} invalid configs")
    return issues
...
def validate_date_diff_left_expr_is_subtraction(
        ...
        issues.append(
            IntentIssue.make(
                ...
                message=(
                    ...
                    "between two date columns; use date_window for relative date-window filters on a single date column"
                ),
                context={"column": col, "location": context},
            )
        )
    ...
    if issues:
        debug(f"[validation_shape.validate_date_diff_left_expr_is_subtraction] {len(issues)} invalid configs")
    return issues
...
def validate_null_filters(
    ...
    if issues:
        debug(f"[validation_shape.validate_null_filters] FAILED with {len(issues)} issues")
    else:
        debug("[validation_shape.validate_null_filters] PASSED")
    return issues
...
def validate_not_in_negated_list_null(
    ...
    *,
    ...
    if issues:
        ...
        debug(f"[validation_shape.validate_not_in_negated_list_null] FAILED with {len(issues)} issues in {context}")
    return issues
...
def validate_unknown_column_operations(
    ...
    if issues:
        for issue in issues:
            ...
            if code:
                emit_session_refusal_diagnostic(code, issue.message)
        debug(f"[validation_shape.validate_unknown_column_operations] FAILED with {len(issues)} issues in {context}")
    return issues
    ...
    for fp in where_params:
        ...
        if col_meta:
            if col_meta.is_foreign_key and col_meta.value_type in {"integer", "number"}:
                ...
                debug(f"[validation_shape.validate_where_value_type_alignment] string value on FK int column {col}")
            continue
        if table_name in cte_outputs:
            cte_cols = cte_outputs[table_name]
            cte_meta = cte_cols.get(col_name) or cte_cols.get(col_name.lower())
...
def validate_no_between_ops(
    ...
    if issues:
        debug(f"[validation_shape.validate_no_between_ops] FAILED with {len(issues)} issues in {context}")
    return issues
...
def _selectability_exempt_qualified_refs(expr: NormalizedExpr, schema: SchemaGraph) -> set[str]:
...
def _selectability_issues_for_normalized_expr(
    ...
    exempt = _selectability_exempt_qualified_refs(expr, schema)
    seen_qc: set[str] = set()
...
def column_name_reserved_for_anti_join(col_name: str) -> bool:
    return col_name.endswith(ANTI_JOIN_PRESENCE_COLUMN_SUFFIX)


def select_col_column_metadata(
    sc: SelectCol,
    schema: SchemaGraph,
    cte_outputs: dict[str, dict[str, CteOutputColumnMeta]],
) -> ColumnMetadata | None:
...
def types_compatible_for_projection(
...
def semi_join_key_shape(cte: RuntimeCteStep, schema: SchemaGraph) -> bool:
    select_cols = cte.select_cols or []
    if not select_cols:
        return False
...
def positional_projection_type_check(
...
def join_sig_left_tables(signature: list[str]) -> set[str]:
    lefts: set[str] = set()
...
def validate_preserve_tables(
    ...
    *,
    ...
    left_tables = join_sig_left_tables(sig)
...
def parse_signature_segments(signature: list[str]) -> list[tuple[str, str, list[str], list[str]]]:
...
def _join_step_multiplies_table(
    ...
    if phys_table_key(table) != phys_table_key(paired_tbl):
        if schema is not None:
            if notes_ratchet_multiplies_table(schema, table, join_tbl, join_cols, paired_tbl, paired_cols):
                return True
        return False
    if schema is not None:
        if notes_ratchet_multiplies_table(schema, table, join_tbl, join_cols, paired_tbl, paired_cols):
...
def multiplying_edges_for_table(
    ...
    *,
    ...
    segments = parse_signature_segments(signature)
    if not segments:
        return []
...
def expr_has_unqualified_count_star(expr: NormalizedExpr | None) -> bool:
    ...
    for comp_tbl in sorted(comparison_only, key=str.lower):
        ...
        if _path_uses_semantic_overlap(path_edges):
            msg = (
                ...
                + ". The comparison does not imply a relationship. Declare foreign_keys_add or a semantic override when the relationship is real."
            )
            issues.append(
                IntentIssue.make(
...
def probe_output_column_metadata_at(
    probe_cte: RuntimeCteStep,
    index: int,
    schema: SchemaGraph,
    cte_outputs: dict[str, dict[str, CteOutputColumnMeta]],
    ...
    if index < len(select_cols):
        return select_col_column_metadata(select_cols[index], schema, cte_outputs)
    return None
...
def qualifies_as_semi_join_probe(
    ...
    for sc in cte.select_cols or []:
        if sc.is_aggregated or (sc.expr and sc.expr.agg_func):
            return False
    if semi_join_key_shape(cte, schema):
        return True
    cte_outputs = build_cte_outputs_map(intent)
    intersection_shape, _ = positional_projection_type_check(
...
def validate_table_scope_repairs(intent: RuntimeIntent) -> list[IntentIssue]:
    """Surface meaningful table-scope repairs as semantic warnings. Routine ``unreferenced_table`` prune/reconcile removals stay debug- only; they are not user-facing semantic warnings."""
    issues: list[IntentIssue] = []
