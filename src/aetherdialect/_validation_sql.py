...
from ._constants import (
    ...
    EXPLAIN_VALIDATION_MESSAGES,
    MASTER_AETHERSPACE_NAME,
    MAX_PREDICATE_NESTING_DEPTH,
    UNBOUND_PYFORMAT_PLACEHOLDER_RE,
)
from ._constants_runtime import PERMISSION_DENIED_USER_MESSAGE
...
from ._sql_gen import (
    classify_cte_emission,
    probe_cte_names,
)
from ._utils import (
    build_case_folded_index,
    debug,
    emit_sql_execution_audit,
    notify,
    pipeline_trace,
...
from ._validation_shape import (
    column_name_reserved_for_anti_join,
    join_sig_left_tables,
    positional_projection_type_check,
    probe_output_column_metadata_at,
    select_col_column_metadata,
    semi_join_key_shape,
    types_compatible_for_projection,
...
def _classify_explain_sql_failure(message: str) -> str:
    """Bucket database ``EXPLAIN`` failures for seed-warmup telemetry and policy. Returns one of ``explain_transient``, ``explain_schema``, ``explain_semantic``, ``explain_cost``, or ``explain_failed``."""
    low = message.lower()
    cost_markers = (
        "cost gate",
        "explain cost",
        "cost exceeded",
        "cost cap",
        "explain_cost_exceeded",
    )
    if any(x in low for x in cost_markers):
        return "explain_cost"
    transient_markers = (
        "timeout",
        "timed out",
...
def _classify_explain_error(dialect_name: str, exc_text: str) -> FailureCategory:
    ...
    if code == "explain_cost":
        return FailureCategory.EXECUTION_COST_EXCEEDED
    return FailureCategory.EXECUTION_EXPLAIN_FAILED


def _format_explain_validation_error(code: str, raw: str) -> str:
    _ = raw
    message = EXPLAIN_VALIDATION_MESSAGES.get(code) or EXPLAIN_VALIDATION_MESSAGES["explain_failed"]
...
def _sqlglot_select_has_forbidden_sql(select: exp.Select, *, allow_union: bool = False) -> bool:
    ...
    if not allow_union:
        forbidden_nodes = (exp.Union, *forbidden_nodes)
...
def _sqlglot_tree_has_forbidden_sql(node: Any, *, allow_union: bool = False) -> bool:
    """Return True when a sqlglot SELECT or UNION tree contains forbidden constructs."""
    if isinstance(node, exp.Select):
        return _sqlglot_select_has_forbidden_sql(node, allow_union=allow_union)
    if isinstance(node, exp.Union):
        ...
        if left is None or right is None:
            return True
        return _sqlglot_tree_has_forbidden_sql(left, allow_union=allow_union) or _sqlglot_tree_has_forbidden_sql(
            right, allow_union=allow_union
        )
    return True


def _pg_parsed_has_forbidden_sql(parsed: Any, *, sql: str, sqlglot_dialect: str, allow_union: bool = False) -> bool:
    ...
    return _sqlglot_tree_has_forbidden_sql(tree, allow_union=allow_union)


def _parsed_ast_has_forbidden_sql(parsed: Any, dialect: Dialect, *, sql: str, allow_union: bool = False) -> bool:
    """Structural refusal list equivalent to PolicyConfig.FORBIDDEN_SQL."""
    if isinstance(parsed, (exp.Select, exp.Union)):
        return _sqlglot_tree_has_forbidden_sql(parsed, allow_union=allow_union)
    dialect_name = getattr(dialect, "name", None)
    sqlglot_dialect = getattr(dialect, "sqlglot_dialect", None)
    if dialect_name == "postgresql" and isinstance(sqlglot_dialect, str):
        return _pg_parsed_has_forbidden_sql(parsed, sql=sql, sqlglot_dialect=sqlglot_dialect, allow_union=allow_union)
    return False


def _enforce_select_only(sql: str, dialect: Dialect, *, allow_union: bool = False) -> tuple[bool, str]:
    """Check whether SQL is a safe SELECT-only statement."""
    s = sql.lower().strip()
    for p in PolicyConfig.FORBIDDEN_SQL:
        if allow_union and p == r"\bUNION\b":
            continue
        if re.search(p, s, re.IGNORECASE):
            return False, "forbidden_sql"
    parsed = dialect.parse_select(sql)
    if parsed is None and allow_union:
        sqlglot_dialect = getattr(dialect, "sqlglot_dialect", None)
        try:
            tree = sqlglot.parse_one(sql, dialect=sqlglot_dialect)
        except Exception:
        ...
        return False, "not_select"
    try:
        if _parsed_ast_has_forbidden_sql(parsed, dialect, sql=sql, allow_union=allow_union):
            return False, "forbidden_sql"
...
def validate_sql(
    ...
    *,
    ...
    debug(f"[validation_sql.validate_sql] checking SQL length={len(sql)}")
    pipeline_trace("validation_sql.validate_sql.input", lambda: stable_json({"sql": sql, "params": params or {}}))
    if UNBOUND_PYFORMAT_PLACEHOLDER_RE.search(sql):
        debug("[validation_sql.validate_sql] unbound pyformat placeholder detected")
        pipeline_trace("validation_sql.validate_sql.FAILED_unbound_placeholder", lambda: sql)
        return (
            False,
            "unbound_placeholder",
    ...
    ok, reason = _enforce_select_only(sql, dialect, allow_union=allow_union)
    if not ok:
        debug(f"[validation_sql.validate_sql] enforce_select_only FAILED: {reason}")
        pipeline_trace("validation_sql.validate_sql.FAILED_enforce_select_only", lambda: f"reason={reason}\n{sql}")
    ...
    if ast_diags:
        ...
        debug(f"[validation_sql.validate_sql] AST validation failed: {ast_err}")
        pipeline_trace("validation_sql.validate_sql.FAILED_ast", lambda: f"ast_err={ast_err}\n{sql}")
        return (
            False,
    ...
    debug("[validation_sql.validate_sql] structural validation succeeded")
    ...
    if dialect.can_explain():
        ...
        if not ok:
            ...
            cost_diag = next(
                (d for d in explain_diags if d.code == SqlDiagnosticCode.EXPLAIN_COST_EXCEEDED),
                None,
            )
            if cost_diag is not None:
                code = "explain_cost"
                formatted = _format_explain_validation_error(code, raw)
                return (False, formatted, FailureCategory.EXECUTION_COST_EXCEEDED, list(explain_diags))
            code = _classify_explain_sql_failure(raw)
            formatted = _format_explain_validation_error(code, raw)
            ecat = _classify_explain_error(getattr(dialect, "name", ""), raw)
            debug(f"[validation_sql.validate_sql] explain_sql failed ({code}): {explain_err}")
            pipeline_trace(
                "validation_sql.validate_sql.FAILED_explain",
        ...
        debug("[validation_sql.validate_sql] explain_sql passed")
    pipeline_trace("validation_sql.validate_sql.PASSED", lambda: sql)
...
def temporary_dialect_member_limits(
    ...
    *,
    ...
    """Apply per-member execution limits on *dialect* for the duration of validation."""
...
def execute_guarded_sql(
    ...
    *,
    ...
    if schema is not None:
        schema_hash_val = str(getattr(schema, "effective_structural_hash", "") or "").strip() or None
    started = time.perf_counter()
    if timeout_ms is not None and backend is not None:
        rows = list(backend.fetch_rows(sql, exec_params, timeout_ms=int(timeout_ms)))
    else:
        rows = dialect.execute(sql, exec_params)
    elapsed_ms = max(0, int((time.perf_counter() - started) * 1000))
    emit_sql_execution_audit(
        statement_hash=sha256(sql),
        row_count=len(rows),
...
def _validate_main_query_cte_usage(
    ...
    debug(f"[validation_sql.validate_main_query_cte_usage] checking main query uses CTEs: {list(cte_outputs.keys())}")
    if not cte_outputs:
        return issues
    ...
    if unreferenced_ctes and cte_steps:
        ...
        debug(f"[validation_sql.validate_main_query_cte_usage] unreferenced CTEs: {unreferenced_ctes}")
    ...
    for sc in intent.select_cols or []:
        ...
        if table_ref.lower() in cte_names_lower:
            ...
            if col_name.lower() not in {c.lower() for c in cte_cols}:
                ...
                debug(f"[validation_sql.validate_main_query_cte_usage] column {col_name} not in CTE {table_ref}")
    ...
    for col_name, source in column_map.items():
        if source.lower() in cte_names_lower:
            ...
            if col_name.lower() not in {c.lower() for c in cte_cols}:
                ...
                debug(f"[validation_sql.validate_main_query_cte_usage] column_map {col_name} not in CTE {source}")
    for fp in PredicateGroup.where_leaves(intent.where) or []:
        ...
        if table_ref.lower() in cte_names_lower:
            ...
            if col_name.lower() not in {c.lower() for c in cte_cols}:
                ...
                debug(f"[validation_sql.validate_main_query_cte_usage] filter {col_name} not in CTE {table_ref}")
    if issues:
        debug(f"[validation_sql.validate_main_query_cte_usage] found {len(issues)} issues")
    else:
        debug("[validation_sql.validate_main_query_cte_usage] all CTE references valid")
    return issues
...
def _validate_cte_output_types(cte_steps: list[RuntimeCteStep], schema: SchemaGraph) -> list[IntentIssue]:
    ...
    debug(f"[validation_sql.validate_cte_output_types] validating {len(cte_steps)} CTE output types")
        ...
        for sc in select_cols:
            ...
            if agg_table in cte_output_types:
                ...
                if col_type and col_type not in ("integer", "number"):
                    ...
                    debug(
                        f"[validation_sql.validate_cte_output_types] type mismatch: {agg_func} on {col_name}({col_type})"
                    )
    if issues:
        debug(f"[validation_sql.validate_cte_output_types] found {len(issues)} type issues")
    else:
        debug("[validation_sql.validate_cte_output_types] all CTE output types valid")
    return issues
...
def _validate_cte_cardinality(cte_steps: list[RuntimeCteStep]) -> list[IntentIssue]:
    ...
    debug(f"[validation_sql.validate_cte_cardinality] validating {len(cte_steps)} CTE cardinalities")
    for cte in cte_steps:
    ...
    for cte in cte_steps:
        ...
        if grain == "scalar" and expected != "one":
            ...
            debug(f"[validation_sql.validate_cte_cardinality] scalar CTE with expected_rows={expected}")
        if grain == "scalar" and cte.limit is not None and cte.limit != 1:
            ...
            debug(f"[validation_sql.validate_cte_cardinality] scalar CTE with multi-row limit={cte.limit}")
        ...
        if cte.limit == 1 and expected != "one":
            ...
            debug(f"[validation_sql.validate_cte_cardinality] LIMIT 1 CTE with expected_rows={expected}")
        for table in cte.tables or []:
            if table in cte_expected_rows:
                ...
                if expected in {"few", "many"} and dep_expected == "one":
                    debug(
                        f"[validation_sql.validate_cte_cardinality] cardinality note dropped: {cte_name}({expected}) <- {table}({dep_expected})"
                    )
    if issues:
        debug(f"[validation_sql.validate_cte_cardinality] found {len(issues)} cardinality issues")
    else:
        debug("[validation_sql.validate_cte_cardinality] all cardinalities valid")
    return issues
...
def validate_cte_emission_reclassification(intent: RuntimeIntent, schema: SchemaGraph) -> list[IntentIssue]:
    """Emit diagnostics when author-owned emission disagrees with engine classification."""
    issues: list[IntentIssue] = []
    for cte in intent.cte_steps or []:
        ...
        if declared_label not in {"semi_join", "anti_join"}:
            continue
        derived = classify_cte_emission(cte, intent, schema)
        if declared == derived or declared_label == (
            derived.value if isinstance(derived, CteEmissionKind) else str(derived)
        ):
            continue
...
def validate_semantics(
    ...
    *,
    ...
    debug(f"[validation_sql.validate_semantics] post_binding={post_binding}")
    debug("[validation_sql.validate_semantics] running semantic validation suite")
    pipeline_trace("validation_sql.validate_semantics.input_intent", lambda: stable_json(intent.to_dict()))
    if not intent.tables:
        ...
        debug("[validation_sql.validate_semantics] tables empty")
    ...
    if all_issues:
        debug(f"[validation_sql.validate_semantics] found {len(all_issues)} total issues")
    else:
        debug("[validation_sql.validate_semantics] all validations passed")
    for idx, iss in enumerate(all_issues):
        debug(
            f"[validation_sql.validate_semantics] issue[{idx}]: "
        ...
        pipeline_trace(f"validation_sql.validate_semantics.issue[{idx}]", _issue_trace_body)
    result = IntentValidationResult(issues=all_issues)
    pipeline_trace("validation_sql.validate_semantics.result", lambda: stable_json(result.to_dict()))
    return result
...
def curated_warmup_post_binding_issues(intent: RuntimeIntent, schema: SchemaGraph, final_sql: str) -> list[str]:
    """Post-substitution parity checks mirroring :func:`post_processing_revalidation_passes` without LLM use."""
    _ = final_sql
    msgs: list[str] = []
...
def validate_execution_parameters(intent: RuntimeIntent, schema: SchemaGraph) -> list[IntentIssue]:
    ...
    issues: list[IntentIssue] = []
    cte_outputs = build_cte_outputs_map(intent)
    allowed_tables = set(intent.tables or [])
    issues.extend(
        validate_filters_schema(
            PredicateGroup.where_leaves(intent.where) or [],
...
def validate_cte_emission_shapes(intent: RuntimeIntent, schema: SchemaGraph) -> list[IntentIssue]:
    ...
    for cte in intent.cte_steps or []:
        ...
        for out_name in cte.output_columns or []:
            if column_name_reserved_for_anti_join(out_name):
                issues.append(
                    IntentIssue.make(
        ...
        if emission == "semi_join":
            ...
            if not has_aggregate:
                key_shape = semi_join_key_shape(cte, schema)
                intersection_shape = False
                intersection_unresolvable = False
                if not key_shape:
                    intersection_shape, intersection_unresolvable = positional_projection_type_check(
        ...
        if emission == "anti_join":
            for fp in PredicateGroup.where_leaves(cte.where) if cte.where else []:
                ...
                if left and column_name_reserved_for_anti_join(left.rsplit(".", 1)[-1]):
                    issues.append(
                        IntentIssue.make(
    ...
    if anti_in_main and intent.select_cols:
        for probe in anti_in_main:
            ...
            for idx, sc in enumerate(outer_cols):
                outer_meta = select_col_column_metadata(sc, schema, cte_outputs)
                probe_meta = probe_output_column_metadata_at(probe_cte, idx, schema, cte_outputs)
                compat = types_compatible_for_projection(outer_meta, probe_meta)
                if compat is None:
...
def validate_probe_cte_anchor_placement(
    ...
    *,
    ...
    def _left_operand_issue(probe: str, signature: list[str], context: str) -> IntentIssue | None:
        left_tables = join_sig_left_tables(signature)
...
def validate_replay_join_semantics(
    intent: RuntimeIntent,
    schema: SchemaGraph,
