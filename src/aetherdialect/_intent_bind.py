...
from ._intent_normalize import (
    ...
    collect_number_typed_predicate_param_keys,
    collect_numeric_predicate_param_keys_from_case_registry,
    cols_from_named_registries,
    cols_from_select_col,
    count_wraps_multi_arg_concat,
    is_expr_date,
    is_expr_string,
    normalized_expr_is_keyword_leaf,
)
...
from ._validation_shape import validate_cte_limits
    ...
    for cte in cte_steps:
        ...
        new_cte_steps.append(
            replace(
                ...
                group_by_cols=c_gb,
                order_by_cols=c_ob,
                where=PredicateGroup.rebuild_from_leaves(cte.where, c_fp),
                having=PredicateGroup.rebuild_from_leaves(cte.having, c_hp),
...
def qualify_count_star_mulgroups(intent: RuntimeIntent, schema_graph: SchemaGraph) -> RuntimeIntent:
    ...
    if main_changed:
        ...
        intent = replace(
            ...
            having=PredicateGroup.rebuild_from_leaves(intent.having, main_having),
        ...
        if cte_changed:
            ...
            new_steps.append(
                replace(
                    ...
                    having=PredicateGroup.rebuild_from_leaves(cte.having, cte_having),
...
def _forward_links(items: Sequence[WhereParam | HavingParam]) -> list[str]:
    """Return AND connectors for a flat predicate list."""
    n = len(items)
    if n == 0:
        return []
    return ["AND"] * n
...
def _dedup_predicate_group(group: PredicateGroup | None) -> PredicateGroup | None:
    ...
    def keep(pred: WhereParam | HavingParam) -> bool:
        key = PredicateGroup.leaf_identity_key(pred)
        if key in seen:
    ...
    kept_groups_list: list[PredicateGroup] = []
    for child in group.groups:
        mapped = _dedup_predicate_group(child)
        if mapped is None:
            continue
        gkey = PredicateGroup.structural_key(mapped)
        if gkey in seen_groups:
            debug(f"[intent_resolve.dedup_predicate_group] dropping duplicate group: {gkey}")
            continue
        seen_groups.add(gkey)
        kept_groups_list.append(mapped)
    result = PredicateGroup(op=group.op, predicates=kept_preds, groups=tuple(kept_groups_list))
    return None if result.is_empty() else result
...
def _intent_is_scalar_shaped(intent: RuntimeIntent) -> bool:
    """Return True when every select is aggregated and there is no GROUP BY."""
    if intent.group_by_cols:
        return False
    select_cols = list(intent.select_cols or [])
    if not select_cols:
        return False
    return all(_select_carries_aggregation(sc, intent.window_registry, intent.case_registry) for sc in select_cols)
...
def enforce_grain_consistency(intent: RuntimeIntent, schema_graph: SchemaGraph) -> RuntimeIntent:
    """Align grain labels with structure and infer GROUP BY for mixed agg/non-agg selects."""
    if _intent_is_scalar_shaped(intent):
        if (intent.grain or "") != "scalar":
            return replace(intent, grain="scalar")
        return intent
    ...
    if not group_by:
        ...
        if has_agg and not non_agg:
            if (intent.grain or "") != "scalar":
                return replace(intent, grain="scalar")
            return intent
        if not (has_agg and non_agg):
            grain = (intent.grain or "row_level").strip().lower()
            if grain in {"scalar", "grouped"} and not has_agg:
                return replace(intent, grain="row_level")
            if grain not in {"row_level", "grouped", "scalar"}:
                return replace(intent, grain="row_level")
            return intent
...
def enforce_cte_grain_consistency(cte: RuntimeCteStep) -> RuntimeCteStep:
    ...
    if has_agg:
        ...
        return cte
    grain = (cte.grain or "row_level").strip().lower()
    if grain in {"scalar", "grouped"}:
        return replace(cte, grain="row_level")
    if grain not in {"row_level", "grouped", "scalar"}:
        return replace(cte, grain="row_level")
    return cte
...
def normalize_cte_names(intent: RuntimeIntent) -> RuntimeIntent:
    """Rename CTEs to sequential ``cteN`` names and rewrite all references. Premature ``cteN`` tokens in ``tables`` (compose anticipating canonical names before steps are renamed) are forward refs to the step that owns that slot, not physical table names that block assignment."""
    cte_steps = intent.cte_steps or []
    if not cte_steps:
        return intent

    old_to_new: dict[str, str] = {}
    for index, cte in enumerate(cte_steps, start=1):
        old_to_new[cte.cte_name] = f"cte{index}"
    next_n = len(cte_steps) + 1
    for planner_name in intent.interpret_cte_names or []:
        pname = (planner_name or "").strip()
        if pname and pname not in old_to_new:
            old_to_new[pname] = f"cte{next_n}"
            next_n += 1

    build_case_folded_index(old_to_new.keys(), kind="CTE name")
    old_to_new_ci: dict[str, str] = {k.lower(): v for k, v in old_to_new.items()}
    assigned_names = {v.lower() for v in old_to_new.values()}
    for token in assigned_names:
        old_to_new_ci.setdefault(token, token)
    ...
    for cte in cte_steps:
        ...
        new_cte = replace(
            ...
            having=PredicateGroup.rebuild_from_leaves(cte.having, new_having),
    ...
    return replace(
        ...
        where=PredicateGroup.rebuild_from_leaves(intent.where, new_main_filters),
        having=PredicateGroup.rebuild_from_leaves(intent.having, new_main_having),
        column_map=new_main_column_map,
        cte_steps=new_cte_steps,
        window_registry=_map_wr(intent.window_registry),
        interpret_cte_names=[c.cte_name for c in new_cte_steps if c.cte_name],
    )
    ...
    for cte in intent.cte_steps or []:
        new_cte_steps.append(
            replace(
                ...
                having=PredicateGroup.rebuild_from_leaves(
                    cte.having,
    ...
    for cte in intent.cte_steps or []:
        ...
        new_cte_steps.append(
            replace(
                ...
                where=PredicateGroup.rebuild_from_leaves(cte.where, cte_filters),
                having=PredicateGroup.rebuild_from_leaves(cte.having, cte_having),
            )
        )
    return replace(
...
def enforce_case_branch_param_keys(intent: RuntimeIntent) -> RuntimeIntent:
    """Guarantee every CASE branch condition has a SQL-renderable binding. For each branch whose operator is not a null-check, ensures either ``right_expr`` or ``param_key`` is set; allocates a fresh ``cb*`` ``param_key`` otherwise so :func:`render_case_branch_sql` cannot raise."""
    next_idx = 1
...
def literal_in_logical_prose(logical: LogicalIntent, token: str) -> bool:
    """Return whether a literal token appears as a case-insensitive. substring in interpret prose."""
...
def attribute_post_compose_issue(issue: IntentIssue, logical: LogicalIntent) -> IntentIssue:
    """Override ``responsible_stage`` for literal-bearing semantic. issues using interpret prose."""
...
def _normalize_sc_pk_distinct(sc: SelectCol, schema_graph: SchemaGraph) -> SelectCol:
    ...
    if not g0 or not g0.distinct:
        return sc
    if count_wraps_multi_arg_concat(e):
        return sc
    col = e.primary_term
...
def _lift_distinct_from_select_col(sc: SelectCol) -> tuple[SelectCol, bool]:
    """Strip stray DISTINCT scalar wrappers from non-aggregate groups. With the parser-native schema, DISTINCT lives on ``MulGroup.distinct``; this helper only catches ``DISTINCT(...)`` scalar wrappers that may slip in. Returns ``(sc, changed_flag)``."""
    e = sc.expr
    if not e.add_groups:
        return sc, False
    new_groups = list(e.add_groups)
...
def _align_pred_value_type(
    ...
    if right is not None and normalized_expr_is_keyword_leaf(right):
    ...
    if (
        left_numeric
        and right_numeric
        and not is_expr_string(left, schema, cte_steps)
        and not is_expr_string(right, schema, cte_steps)
    ):
        target = "number"
        if current != target:
            debug(
    ...
    left_date = is_expr_date(left, schema, cte_steps)
    right_date = right is None or is_expr_date(right, schema, cte_steps)
    ...
    if is_expr_string(left, schema, cte_steps) or is_expr_string(right, schema, cte_steps):
...
def _coerce_boolean_bindings_for_number_typed_where(intent: RuntimeIntent) -> RuntimeIntent:
    keys: set[str] = set()
    for fp in PredicateGroup.where_leaves(intent.where) or []:
        keys.update(collect_number_typed_predicate_param_keys(fp))
    for hp in PredicateGroup.having_leaves(intent.having) or []:
        keys.update(collect_number_typed_predicate_param_keys(hp))
    for cte in intent.cte_steps or []:
        for fp in PredicateGroup.where_leaves(cte.where) or []:
            keys.update(collect_number_typed_predicate_param_keys(fp))
        for hp in PredicateGroup.having_leaves(cte.having) or []:
            keys.update(collect_number_typed_predicate_param_keys(hp))
        keys.update(collect_numeric_predicate_param_keys_from_case_registry(cte.case_registry))
    keys.update(collect_numeric_predicate_param_keys_from_case_registry(intent.case_registry))
    ...
    for cte in intent.cte_steps or []:
        ...
        new_ctes.append(
            replace(
                cte,
                where=PredicateGroup.rebuild_from_leaves(cte.where, nf),
                having=PredicateGroup.rebuild_from_leaves(cte.having, nh),
