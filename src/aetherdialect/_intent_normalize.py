...
from ._validation_shape import (
    format_table_scope_repair_message,
...
def _predicate_column_key(pred: WhereParam | HavingParam) -> str:
    return (pred.left_expr.primary_column or expr_canonical_key(pred.left_expr) or "").strip().lower()


def _predicate_resolved_literal(pred: WhereParam | HavingParam, param_values: Mapping[str, Any] | None) -> Any:
    if pred.right_expr is not None:
        return None
    return pred.resolved_value(param_values)


def _simplify_and_equality_clashes(
    preds: list[WhereParam | HavingParam], param_values: Mapping[str, Any] | None
) -> list[WhereParam | HavingParam] | None:
    """Return None when AND equality literals on one column conflict; else preds unchanged structurally."""
    eq_values: dict[str, Any] = {}
    for pred in preds:
        if pred.op != "=" or pred.right_expr is not None:
            continue
        col = _predicate_column_key(pred)
        if not col:
            continue
        value = _predicate_resolved_literal(pred, param_values)
        if value is None:
            continue
        prior = eq_values.get(col)
        if prior is not None and prior != value:
            debug(f"[simplify_predicate_semantics] equality clash on {col!r}: {prior!r} vs {value!r}")
            return None
        eq_values[col] = value
    return preds


def _simplify_and_null_clashes(preds: list[WhereParam | HavingParam]) -> list[WhereParam | HavingParam] | None:
    """Detect ``IS NULL`` vs equality / ``IS NOT NULL`` conflicts under AND."""
    ...
    for pred in preds:
        ...
        elif op == "=" and pred.right_expr is None:
            eq_cols.add(col)
    if null_cols & not_null_cols or null_cols & eq_cols:
        debug("[simplify_predicate_semantics] NULL constraint clash under AND")
        return None
    return preds


def _simplify_and_ranges(
    preds: list[WhereParam | HavingParam], param_values: Mapping[str, Any] | None
) -> list[WhereParam | HavingParam] | None:
    """Tighten numeric/temporal range bounds under AND; return None when empty."""
    ranges: dict[str, dict[str, Any]] = {}
    others: list[WhereParam | HavingParam] = []
    for pred in preds:
        col = _predicate_column_key(pred)
        op = (pred.op or "").strip()
        if not col or op not in RANGE_OPS or pred.right_expr is not None:
            others.append(pred)
            continue
        value = _predicate_resolved_literal(pred, param_values)
        if not isinstance(value, int | float) or isinstance(value, bool):
            others.append(pred)
            continue
        slot = ranges.setdefault(col, {"gt": None, "ge": None, "lt": None, "le": None, "preds": {}})
        slot["preds"][op] = pred
        if op == ">":
            slot["gt"] = value if slot["gt"] is None else max(slot["gt"], value)
        elif op == ">=":
            slot["ge"] = value if slot["ge"] is None else max(slot["ge"], value)
        elif op == "<":
            slot["lt"] = value if slot["lt"] is None else min(slot["lt"], value)
        elif op == "<=":
            slot["le"] = value if slot["le"] is None else min(slot["le"], value)
    tightened: list[WhereParam | HavingParam] = []
    for col, slot in ranges.items():
        lower = None
        lower_inclusive = True
        if slot["gt"] is not None and slot["ge"] is not None:
            if slot["gt"] >= slot["ge"]:
                lower, lower_inclusive = slot["gt"], False
            else:
                lower, lower_inclusive = slot["ge"], True
        elif slot["gt"] is not None:
            lower, lower_inclusive = slot["gt"], False
        elif slot["ge"] is not None:
            lower, lower_inclusive = slot["ge"], True
        upper = None
        upper_inclusive = True
        if slot["lt"] is not None and slot["le"] is not None:
            if slot["lt"] <= slot["le"]:
                upper, upper_inclusive = slot["lt"], False
            else:
                upper, upper_inclusive = slot["le"], True
        elif slot["lt"] is not None:
            upper, upper_inclusive = slot["lt"], False
        elif slot["le"] is not None:
            upper, upper_inclusive = slot["le"], True
        if lower is not None and upper is not None:
            if lower > upper or (lower == upper and not (lower_inclusive and upper_inclusive)):
                debug(f"[simplify_predicate_semantics] empty range on {col!r}")
                return None
        if lower is not None:
            op = ">=" if lower_inclusive else ">"
            base = slot["preds"].get(op) or next(iter(slot["preds"].values()))
            tightened.append(replace(base, op=op, raw_value=lower))
        if upper is not None:
            op = "<=" if upper_inclusive else "<"
            base = slot["preds"].get(op) or next(iter(slot["preds"].values()))
            tightened.append(replace(base, op=op, raw_value=upper))
    return others + tightened


def _collapse_or_equalities_to_in(
    preds: list[WhereParam | HavingParam], param_values: Mapping[str, Any] | None
) -> list[WhereParam | HavingParam]:
    """Rewrite ``col=A OR col=B`` leaf sets under an OR into a single ``IN`` when safe."""
    buckets: dict[tuple[str, str], list[WhereParam | HavingParam]] = {}
    passthrough: list[WhereParam | HavingParam] = []
    for pred in preds:
        col = _predicate_column_key(pred)
        if pred.op != "=" or pred.right_expr is not None or not col:
            passthrough.append(pred)
            continue
        value = _predicate_resolved_literal(pred, param_values)
        if value is None:
            passthrough.append(pred)
            continue
        buckets.setdefault((col, pred.value_type or ""), []).append(pred)
    out: list[WhereParam | HavingParam] = list(passthrough)
    for (_col, _vt), group in buckets.items():
        if len(group) == 1:
            out.append(group[0])
            continue
        values: list[Any] = []
        for pred in group:
            ...
            if value not in values:
                values.append(value)
        base = group[0]
        out.append(replace(base, op="in", raw_value=values, param_key=""))
        debug(f"[simplify_predicate_semantics] collapsed {len(group)} equalities into IN ({len(values)} values)")
    return out


def _simplify_predicate_group_semantics(
    group: PredicateGroup | None, param_values: Mapping[str, Any] | None
) -> PredicateGroup | None:
    """Apply structural semantic folds while preserving AND/OR structure."""
    ...
    nested = tuple(
        ...
        if (simplified := _simplify_predicate_group_semantics(child, param_values)) is not None
    )
    preds = list(group.predicates)
    if group.op == "and":
        if _simplify_and_equality_clashes(preds, param_values) is None:
            return PredicateGroup.contradiction()
        if _simplify_and_null_clashes(preds) is None:
            return PredicateGroup.contradiction()
        ranged = _simplify_and_ranges(preds, param_values)
        if ranged is None:
            return PredicateGroup.contradiction()
        preds = ranged
        if any(child.is_contradiction() for child in nested):
            return PredicateGroup.contradiction()
        nested = tuple(child for child in nested if not child.is_contradiction())
    else:
        preds = _collapse_or_equalities_to_in(preds, param_values)
        nested = tuple(child for child in nested if child is not None and not child.is_empty())
        if not preds and not nested:
            return PredicateGroup.contradiction()
    result = PredicateGroup(op=group.op, predicates=tuple(preds), groups=nested)
    if result.is_empty():
        return None
    return PredicateGroup.collapse_trivial(PredicateGroup.absorb(PredicateGroup.flatten(result)))


def simplify_predicate_semantics(intent: RuntimeIntent) -> RuntimeIntent:
    """Fold equality clashes, NULL clashes, range intersection, and OR- equality→IN on predicate trees."""
    pv = intent.param_values or {}
    new_where = _simplify_predicate_group_semantics(intent.where, pv)
    new_having = _simplify_predicate_group_semantics(intent.having, pv)
    ...
    for cte in intent.cte_steps or []:
        ...
        new_ctes.append(
            replace(
                cte,
                where=_simplify_predicate_group_semantics(cte.where, cte_pv),
                having=_simplify_predicate_group_semantics(cte.having, cte_pv),
            )
        )
    return replace(intent, where=new_where, having=new_having, cte_steps=new_ctes)
...
def normalized_expr_is_keyword_leaf(expr: NormalizedExpr) -> bool:
...
def _promote_temporal_keyword_where(filters: list[WhereParam]) -> tuple[list[WhereParam], bool]:
    ...
    changed = False
    for fp in filters:
        if fp.right_expr is not None and normalized_expr_is_keyword_leaf(fp.right_expr):
...
def _promote_temporal_keyword_havings(havings: list[HavingParam]) -> tuple[list[HavingParam], bool]:
    ...
    changed = False
    for hp in havings:
        if hp.right_expr is not None and normalized_expr_is_keyword_leaf(hp.right_expr):
...
def _where_right_expr_redundant_with_value(fp: WhereParam) -> bool:
    if fp.right_expr is None or fp.raw_value is None:
        return False
    if normalized_expr_is_keyword_leaf(fp.right_expr):
        return False
...
def _having_right_expr_redundant_with_value(hp: HavingParam) -> bool:
    if hp.right_expr is None or hp.raw_value is None:
        return False
    if normalized_expr_is_keyword_leaf(hp.right_expr):
        return False
...
def repair_fk_where_type_mismatch(intent: RuntimeIntent, schema_graph: SchemaGraph) -> RuntimeIntent:
    ...
    for cte in intent.cte_steps or []:
        ...
        if c:
            new_cte_steps.append(replace(cte, where=PredicateGroup.rebuild_from_leaves(cte.where, cte_filters)))
            cte_changed = True
        else:
    ...
    result = intent
    if main_changed:
        result = replace(result, where=PredicateGroup.rebuild_from_leaves(intent.where, main_filters))
    if cte_changed:
        result = replace(result, cte_steps=new_cte_steps)
    return result
...
def _expand_fk_select_to_descriptive_tables_sel(
    ...
    for sc in select_cols or []:
        ...
        if not fk_target:
            ...
            continue
        target_table, _ = fk_target
        descs = best_descriptive_columns(target_table, schema_graph, existing_terms, max_count=2)
        if not descs:
            new_select.append(sc)
            continue
        for desc in descs:
...
def strip_impossible_having(intent: RuntimeIntent) -> RuntimeIntent:
    ...
    def _keep(hp: WhereParam | HavingParam) -> bool:
        return isinstance(hp, HavingParam) and not _is_impossible_having(hp)

    kept_main, dropped_main = PredicateGroup.partition(intent.having, _keep)
    main_changed = dropped_main is not None
    if main_changed:
        removed = dropped_main.leaf_count() if dropped_main is not None else 0
        debug(f"[strip_impossible_having] removed {removed} impossible HAVING condition(s)")

    new_cte_steps = []
    cte_changed = False
    for cte in intent.cte_steps or []:
        kept_cte, dropped_cte = PredicateGroup.partition(cte.having, _keep)
        if dropped_cte is not None:
            cte_changed = True
            new_cte_steps.append(replace(cte, having=kept_cte))
        else:
            new_cte_steps.append(cte)
...
def count_wraps_multi_arg_concat(expr: NormalizedExpr) -> bool:
...
def is_expr_date(
    ...
    if (expr.agg_func or "").lower() in NON_NUMERIC_AGGS_FOR_DATES:
        vt = expr_primary_column_value_type(expr, schema, cte_steps)
...
def is_expr_string(
    ...
    if expr is None:
        return False
    vt = expr_primary_column_value_type(expr, schema, cte_steps)
    if vt and vt in STRING_VALUE_TYPES:
        return True
    return False


def collect_number_typed_predicate_param_keys(pred: WhereParam | HavingParam) -> set[str]:
...
def collect_numeric_predicate_param_keys_from_case_registry(registry: Sequence[CaseRegistryStep] | None) -> set[str]:
    keys: set[str] = set()
    if not registry:
        return keys
    for step in registry:
        ...
        for br in cw.branches:
            cond = br.condition
            keys.update(collect_number_typed_predicate_param_keys(cond))
    return keys
    ...
    for issue in unreachable:
        ...
        if root and target:
            return (
                ...
                "relates them. Repair removed a table to clear the error, which would answer a different question. Declare foreign_keys_add or a semantic neighbour override when the relationship is real."
            )
        return "Repair removed a table while a join-unreachable error was open, which would answer a different question. Declare foreign_keys_add or a semantic neighbour override when the relationship is real."
    return None
...
def prune_unreferenced_registries(intent: RuntimeIntent) -> RuntimeIntent:
    """Drop window/case registry rows that nothing in the same scope references."""
    main_wr, main_cr = where_scope_registries_to_referenced(
        select_cols=intent.select_cols,
        order_by_cols=intent.order_by_cols,
        group_by_cols=intent.group_by_cols,
    ...
    cte_changed = False
    for cte in intent.cte_steps or []:
        wr, cr = where_scope_registries_to_referenced(
            select_cols=cte.select_cols,
            order_by_cols=cte.order_by_cols,
            group_by_cols=cte.group_by_cols,
        ...
        if wr != list(cte.window_registry or []) or cr != list(cte.case_registry or []):
            cte_changed = True
            new_ctes.append(replace(cte, window_registry=wr, case_registry=cr))
        else:
            new_ctes.append(cte)
    if (
        main_wr == list(intent.window_registry or [])
        and main_cr == list(intent.case_registry or [])
        and not cte_changed
    ):
        return intent
    return replace(
        intent,
        window_registry=main_wr,
        case_registry=main_cr,
        cte_steps=new_ctes,
    )
...
def append_table_scope_repairs(
    ...
    *,
    ...
    add_reason: Literal["interpret_align", "expression_reference", "unreferenced_table", "join_bridge"] = (
        "interpret_align"
    ),
    remove_reason: Literal["interpret_align", "expression_reference", "unreferenced_table", "join_bridge"] = (
        "unreferenced_table"
    ),
) -> RuntimeIntent:
...
def table_scope_repair_warning_messages(intent: RuntimeIntent) -> list[str]:
    """Return deduplicated user-facing warning strings for recorded table-scope repairs. Skips routine ``unreferenced_table`` reconcile removals (debug- only)."""
    seen: set[str] = set()
    out: list[str] = []
    for repair in intent.table_scope_repairs or []:
        if repair.reason == "unreferenced_table":
            continue
...
def _normalize_in_types_for_list(filters: list[WhereParam], schema_graph: SchemaGraph) -> tuple[list[WhereParam], bool]:
    ...
    for fp in filters:
        ...
        coerced = [coerce_element(v, dtype, col_meta=col_meta) for v in fp.raw_value]
...
def normalize_in_where_types(intent: RuntimeIntent, schema_graph: SchemaGraph) -> RuntimeIntent:
    """Run IN coercion on main/CTEs, then ``decompose_in_not_in_where``. Filter-only by design; HAVING operates on aggregation results, not ``IN``-list expansion here."""
    ...
    return decompose_in_not_in_where(intent)
    ...
    def process(filters: list[WhereParam]) -> tuple[list[WhereParam], bool]:
        ...
        for fp in filters:
            ...
            if fp.op == "contains":
                ...
                if meta is not None and vt in STRING_VALUE_TYPES:
                    rv = fp.raw_value
...
def intent_expr_terms_blob(expr: NormalizedExpr) -> str:
...
def build_column_term_replacer(
...
def build_table_term_replacer(src_table: str, dst_table: str) -> Callable[[str], str]:
...
def apply_column_replacer_to_intent(intent: RuntimeIntent, replacer: Callable[[str], str]) -> RuntimeIntent:
...
def _repair_unknown_column(intent: RuntimeIntent, schema: SchemaGraph, diag: SqlDiagnostic) -> RuntimeIntent | None:
    ...
    if src_table is not None:
        ...
        if best is None or best == src_column:
            return None
        replacer = build_column_term_replacer(src_table, src_column, src_table, best)
        debug(f"[intent_repair._repair_unknown_column] {src_table}.{src_column} -> {src_table}.{best}")
        return apply_column_replacer_to_intent(intent, replacer)
    ...
    if best_table is None or best_column is None:
        return None
    replacer = build_column_term_replacer(None, src_column, best_table, best_column)
...
def _repair_ambiguous_column(intent: RuntimeIntent, schema: SchemaGraph, diag: SqlDiagnostic) -> RuntimeIntent | None:
    ...
    if chosen is None:
        return None
    replacer = build_column_term_replacer(None, raw, chosen, raw)
...
def _repair_unknown_table(intent: RuntimeIntent, schema: SchemaGraph, diag: SqlDiagnostic) -> RuntimeIntent | None:
    ...
    if new_tables == list(intent.tables or []):
        return None
    replacer = build_table_term_replacer(raw, best)
    rewritten = apply_column_replacer_to_intent(intent, replacer)
...
def _repair_param_binding(intent: RuntimeIntent, _schema: SchemaGraph, diag: SqlDiagnostic) -> RuntimeIntent | None:
    ...
    def _keep(fp: WhereParam | HavingParam) -> bool:
        if not isinstance(fp, WhereParam):
            return True
        keys = {(fp.param_key or "").lower(), (fp.param_key_hi or "").lower()}
        return not (target_low in keys and fp.raw_value is None)

    kept, dropped = PredicateGroup.partition(intent.where, _keep)
    if dropped is None:
        return None
    debug(f"[intent_repair._repair_param_binding] dropped unbound-param filter: {target_low}")
    return replace(intent, where=kept)
...
def expr_primary_column_value_type(
...
def coerce_element(val: Any, data_type: str, *, col_meta: ColumnMetadata | None = None) -> Any:
