...
from ._intent_bind import join_path_key_concrete, join_path_key_runtime, sort_having, sort_where_predicates
from ._llm_provider import LLMProvider
from ._schema_graph import effective_execution_visible_tables
from ._utils import (
...
def build_question_validation_inventory(
    *,
    table_names: frozenset[str] | Sequence[str] | None = None,
    domain_knowledge_keys: frozenset[str] | Sequence[str] | None = None,
) -> dict[str, list[str]]:
    """Build caller-scoped routing inventory: table names and DK concept keys only."""
    tables = sorted({t.strip() for t in (table_names or ()) if isinstance(t, str) and t.strip()})
    keys = sorted({k.strip() for k in (domain_knowledge_keys or ()) if isinstance(k, str) and k.strip()})
    return {"table_names": tables, "domain_knowledge_keys": keys}
...
def validate_question(
    ...
    *,
    ...
    inventory = build_question_validation_inventory(
        table_names=table_names,
        domain_knowledge_keys=domain_knowledge_keys,
    )
    user_message = stable_json({"question": question, "inventory": inventory})
    try:
        result = LLMProvider.json(QUESTION_VALIDATION_SYSTEM, user_message, task="intake_validate")
    ...
    if not isinstance(result, dict):
        raise ValueError(f"validate_question: LLM JSON is not an object; got {type(result).__name__}")
    if "valid_database_question" not in result:
        raise ValueError("validate_question: LLM JSON missing 'valid_database_question'")
    if "query_type" not in result:
        raise ValueError("validate_question: LLM JSON missing 'query_type'")
    valid_raw = result["valid_database_question"]
    query_type_raw = result["query_type"]
    if not isinstance(valid_raw, str):
        raise ValueError(f"validate_question: 'valid_database_question' must be str; got {type(valid_raw).__name__}")
    if not isinstance(query_type_raw, str):
        raise ValueError(f"validate_question: 'query_type' must be str; got {type(query_type_raw).__name__}")
    query_type = query_type_raw.strip().lower()
    if query_type == "allowed":
        query_type = QuestionRoute.ANALYTICAL.value
    valid = valid_raw.strip().lower() == "yes"
    corrected_raw = result.get("corrected", question)
    if corrected_raw is None or corrected_raw == "":
        corrected = question
    elif isinstance(corrected_raw, str):
        corrected = corrected_raw
    else:
        raise ValueError(f"validate_question: 'corrected' must be str when present; got {type(corrected_raw).__name__}")
    if query_type == QuestionRoute.RESTRICTED.value:
        ...
        return QuestionValidationResult(
            accepted=False,
            route=QuestionRoute.INVALID,
            corrected=corrected,
            invalid_kind="conversational",
        )
    elif query_type == "unmappable":
        return QuestionValidationResult(
            accepted=False,
            route=QuestionRoute.INVALID,
            corrected=corrected,
            invalid_kind="unmappable",
        )
    elif not valid:
        invalid_kind = "unmappable" if query_type == "unspecified" else "unmappable"
        return QuestionValidationResult(
            accepted=False,
            route=QuestionRoute.INVALID,
            corrected=corrected,
    ...
    elif query_type == QuestionRoute.SCHEMA_CATALOG.value:
        return QuestionValidationResult(accepted=True, route=QuestionRoute.SCHEMA_CATALOG, corrected=corrected)
    elif query_type == QuestionRoute.DOMAIN_KNOWLEDGE.value:
        return QuestionValidationResult(accepted=True, route=QuestionRoute.DOMAIN_KNOWLEDGE, corrected=corrected)
    elif query_type == QuestionRoute.SCHEMA_AND_KNOWLEDGE.value:
        return QuestionValidationResult(accepted=True, route=QuestionRoute.SCHEMA_AND_KNOWLEDGE, corrected=corrected)
    elif query_type == QuestionRoute.ANALYTICAL.value:
        return QuestionValidationResult(accepted=True, route=QuestionRoute.ANALYTICAL, corrected=corrected)
    elif query_type == "unspecified":
        return QuestionValidationResult(
            accepted=False,
            route=QuestionRoute.INVALID,
            corrected=corrected,
        ...
        raise ValueError(f"validate_question: unexpected query_type {query_type_raw!r}")
...
def normalize_question_via_llm(corrected: str, *, raw_original: str | None = None) -> str:
    ...
    try:
        result = LLMProvider.json(QUESTION_CANONICALIZE_SYSTEM, stable_json(user_obj), task="intake_normalize")
    except LlmJsonExhausted as exc:
        debug(f"[utils.normalize_question_via_llm] llm_json exhausted: {exc}")
        return corrected
    if not isinstance(result, dict):
        raise ValueError(f"normalize_question_via_llm: LLM JSON is not an object; got {type(result).__name__}")
    if "normalized" not in result:
        raise ValueError("normalize_question_via_llm: LLM JSON missing 'normalized' key")
    normalized_raw = result["normalized"]
    if not isinstance(normalized_raw, str):
        raise ValueError(f"normalize_question_via_llm: 'normalized' must be str; got {type(normalized_raw).__name__}")
...
def question_token_fingerprint_from_raw(raw: str) -> str:
    """Fingerprint for a raw question string after :func:`aetherdialect._utils.normalize_question`."""
...
def resolve_caller_visible_tables(
    *,
    ...
    """Return execution-visible table names for template reuse filtering."""
    if visible_tables is not None:
        return visible_tables
    if schema is not None and (schema_context is not None or visible_objects is not None):
        return effective_execution_visible_tables(schema, schema_context, visible_objects)
    return None
...
def template_footprint_tables(template: Template) -> frozenset[str]:
    ...
    tables = {str(t).strip() for t in template.footprint_tables if str(t).strip()}
    if tables:
        return frozenset(tables)
    tables = {str(t).strip() for t in (template.tables_used or ()) if str(t).strip()}
    concrete = template.intent_signature
    if concrete.tables:
        tables.update(str(t).strip() for t in concrete.tables if str(t).strip())
    return frozenset(tables)
...
def template_enumerable_by_caller(
    ...
    *,
    ...
    """Return whether *template* may appear in caller-facing enumeration APIs."""
    if not template_visible_to_callers(template):
        return False
    if visible_tables is None:
        return True
    return template_footprint_tables(template) <= visible_tables
...
def resolve_template_for_question(
    ...
    *,
    template_store: Mapping[str, Any] | None = None,
    visible_tables: frozenset[str] | None = None,
    ...
    if visible_tables is not None:
        templates_list = [t for t in templates_list if template_enumerable_by_caller(t, visible_tables=visible_tables)]
...
def _normalize_cte_steps(steps: Any, available_ctes: dict[str, list[str]] | None = None) -> list[RuntimeCteStep]:
    ...
    for s in steps:
        if isinstance(s, RuntimeCteStep):
            ...
            where_params = PredicateGroup.where_leaves(PredicateGroup.parse_where_field(s)) or []
            having_param = PredicateGroup.having_leaves(PredicateGroup.parse_having_field(s)) or []
...
def intent_key(intent: RuntimeIntent) -> str:
    """SHA-256 of normalised structural intent: tables, selects, filters, group/order/having, CTEs. Omits ``grain``, ``limit``, and raw param values; uses normalised filter/having dicts and CTE key skeletons. Differs from :func:`aetherdialect._intent_loop.intent_similarity`, which scores overlap via weighted clause similarity (including a separate CTE blend) rather than a single hash."""
...
def generate_question(
    ...
    system = (
        ...
        "- Use semantic context to refer to domain concepts naturally\n"
    ...
    if not isinstance(response, dict):
        raise ValueError(f"generate_question: LLM JSON is not an object; got {type(response).__name__}")
    if "question" not in response:
        raise ValueError("generate_question: LLM JSON missing 'question' key")
    question = response["question"]
    if question is not None and not isinstance(question, str):
        raise ValueError(f"generate_question: 'question' must be str; got {type(question).__name__}")
    if "is_realistic" in response:
        ir = response["is_realistic"]
        if isinstance(ir, str):
            ir = str(ir).lower() in ("true", "1", "yes")
        elif not isinstance(ir, bool):
            raise ValueError(f"generate_question: 'is_realistic' must be bool or str; got {type(ir).__name__}")
    else:
        ir = True
    if not ir:
    ...
    if question and isinstance(question, str):
        ...
        return question
    raise ValueError("generate_question: 'question' is empty")
...
def schema_context_enriched_lines_for_tables(schema: SchemaGraph, tables: list[str]) -> str:
    ...
    for table in tables:
        ...
        if not table_meta:
            continue
        lines: list[str] = [f"TABLE {table}"]
        td = str(getattr(table_meta, "description", "") or "").strip()
        if td:
        ...
        for col_name, col_meta in table_meta.columns.items():
            ...
            cd = str(col_meta.description or "").strip()
            if cd:
                piece += f" — {cd}"
...
def generate_warmup_paraphrases_by_style(
    ...
    if not isinstance(response, dict):
        raise ValueError(
            f"generate_warmup_paraphrases_by_style: LLM JSON is not an object; got {type(response).__name__}"
        )
    if "paraphrases_by_style" not in response:
        raise ValueError("generate_warmup_paraphrases_by_style: LLM JSON missing 'paraphrases_by_style'")
    raw = response["paraphrases_by_style"]
    if not isinstance(raw, dict):
        raise ValueError(
            f"generate_warmup_paraphrases_by_style: 'paraphrases_by_style' must be a dict; got {type(raw).__name__}"
        )
    ...
    for style in styles:
        ...
        if not isinstance(vals, list):
            raise ValueError(
                f"generate_warmup_paraphrases_by_style: style {style!r} must map to a list; got {type(vals).__name__}"
            )
        for item in vals:
            if isinstance(item, str) and item.strip():
                ...
                raise ValueError(
                    f"generate_warmup_paraphrases_by_style: paraphrase items must be str; got {type(item).__name__}"
                )
...
def generate_warmup_questions_freeform(
    ...
    if not isinstance(response, dict):
        raise ValueError(
            f"generate_warmup_questions_freeform: LLM JSON is not an object; got {type(response).__name__}"
        )
    if "questions" in response:
        raw = response["questions"]
        if not isinstance(raw, list):
            raise ValueError(
                f"generate_warmup_questions_freeform: 'questions' must be a list; got {type(raw).__name__}"
            )
    elif "question" in response:
        q0 = response["question"]
        if q0 is None or q0 == "":
            raw = []
        elif isinstance(q0, str):
            raw = [q0] if q0.strip() else []
        else:
            raise ValueError(f"generate_warmup_questions_freeform: 'question' must be str; got {type(q0).__name__}")
    else:
        raise ValueError("generate_warmup_questions_freeform: LLM JSON missing 'questions' or 'question'")
    out: list[str] = []
    seen: set[str] = set()
    for item in raw:
        if not isinstance(item, str):
            raise ValueError(
                f"generate_warmup_questions_freeform: question items must be str; got {type(item).__name__}"
            )
        if not item.strip():
            continue
...
def generate_question_from_sql(
    ...
    if not isinstance(response, dict):
        raise ValueError(f"generate_question_from_sql: LLM JSON is not an object; got {type(response).__name__}")
    if "is_realistic" not in response:
        raise ValueError("generate_question_from_sql: LLM JSON missing 'is_realistic'")
    is_realistic = response["is_realistic"]
    if isinstance(is_realistic, bool):
        pass
    elif isinstance(is_realistic, str):
        is_realistic = str(is_realistic).lower() in ("true", "1", "yes")
    else:
        raise ValueError(
            f"generate_question_from_sql: 'is_realistic' must be bool or str; got {type(is_realistic).__name__}"
        )
    ...
    if not is_realistic and intent_source != "sql_history":
        ...
        if raw_cat is None:
            ...
            cat = raw_cat.strip() if raw_cat.strip() in REALISM_DROP_REASON_CATEGORIES else "other"
        else:
            raise ValueError(
                f"generate_question_from_sql: 'drop_reason_category' must be str; got {type(raw_cat).__name__}"
            )
        return {
            "questions": [],
            "question": "",
    ...
    if not phrases:
        if "questions" in response:
            raw_list = response["questions"]
            if not isinstance(raw_list, list):
                raise ValueError(
                    f"generate_question_from_sql: 'questions' must be a list; got {type(raw_list).__name__}"
                )
            for x in raw_list:
                if isinstance(x, str) and x.strip():
                    phrases.append(x.strip())
                elif x is not None and not isinstance(x, str):
                    raise ValueError(f"generate_question_from_sql: question items must be str; got {type(x).__name__}")
        if not phrases:
            if "question" in response:
                ...
                if q0 is None or q0 == "":
                    ...
                    phrases = [q0.strip()]
                elif not isinstance(q0, str):
                    raise ValueError(f"generate_question_from_sql: 'question' must be str; got {type(q0).__name__}")
            elif "questions" not in response:
                raise ValueError("generate_question_from_sql: LLM JSON missing 'questions' and 'question'")
