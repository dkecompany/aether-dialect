"""rental_shop dev tooling: load, ping, extract-csv."""
...
def load_env_file(path: str | Path | None = None, *, override: bool = False) -> Path:
    ...
    env_path = (
        Path(path)
        if path
        else Path(os.environ.get("LIVE_ENV_FILE", str(DEFAULT_ENV_FILE)))
    )
...
def split_block(block: str) -> tuple[str, str, list[str]]:
    ...
    if not name_match:
        raise ValueError(
            f"Could not parse CREATE TABLE name from block: {block[:80]!r}"
        )
...
def integer_columns_for_table(table: str, *, ddl_text: str | None = None) -> frozenset[str]:
    ...
    if _DDL_INTEGER_COLUMNS is None:
        _DDL_INTEGER_COLUMNS = parse_integer_columns_from_ddl(
...
def translate_alter_pk(
    engine: str, table: str, schema: str, pk_cols: list[str]
) -> str | None:
...
def translate_alter_fk(
    engine: str, table: str, schema: str, fk: str, constraint_name: str
) -> str | None:
    ...
    if engine == "snowflake":
        ...
        return (
            f"ALTER TABLE {schema}.{table} ADD CONSTRAINT {constraint_name} {fk_clean}"
        )
    if engine == "databricks":
        fk_clean = re.sub(
            r"\bREFERENCES\s+(\w+)\s*\(",
        ...
        for i in range(len(fk_map.get(table, []))):
            conn.execute(
                text(
                    ...
                    f"DROP CONSTRAINT IF EXISTS {table}_fk_{i}"
                )
            )
...
def _databricks_create_string_staging(
    conn, target: str, stg_table: str, columns: list[str]
) -> None:
...
def _databricks_insert_dataframe(
    ...
    *,
    ...
    for start in range(0, len(frame), chunk_size):
        ...
        values_sql = ", ".join(
            "("
            + ", ".join(_databricks_sql_literal(row[c]) for c in cols)
            + ")"
            for _, row in batch.iterrows()
        )
        try:
            conn.exec_driver_sql(
                f"INSERT INTO {target}.{table} ({col_list}) VALUES {values_sql}"
            )
...
_BQ_TIMESTAMP_COLS = frozenset(
    {"last_update", "rental_date", "return_date", "payment_date"}
)
_BQ_DATE_COLS = frozenset(
    {"create_date", "start_date", "end_date", "ordered_date", "received_date"}
)
...
def _load_bigquery_table(
    client, project: str, dataset: str, table: str, frame: pd.DataFrame
) -> None:
        ...
        if field.field_type in ("TIMESTAMP", "DATETIME"):
            if not pd.api.types.is_datetime64_any_dtype(df[field.name]):
                df[field.name] = pd.to_datetime(
                    df[field.name], format="mixed", errors="coerce", utc=True
                )
            ...
            if pd.api.types.is_datetime64_any_dtype(df[field.name]):
                ...
                df[field.name] = pd.to_datetime(
                    df[field.name], format="mixed", errors="coerce"
                ).dt.date
            ...
            df[field.name] = df[field.name].map(
                lambda v: None if (v is None or pd.isna(v)) else Decimal(str(v))
            )
...
def _ensure_sqlserver_database(runtime_cls, database: str) -> None:
    ...
    with engine.connect() as conn:
        conn.execute(
            text(f"IF DB_ID(N'{database}') IS NULL CREATE DATABASE [{database}]")
        )
...
def _load_duckdb(args: argparse.Namespace) -> None:
    ...
    db_path = _resolve_embedded_db_path(
        DuckDBRuntimeConfig.DATABASE_PATH or ":memory:", "rental_shop.duckdb"
    )
    ...
    try:
        ...
        for table in _TABLE_ORDER:
            ...
            for row in rows:
                values = [
                    _embedded_cell_duckdb(table, col, row.get(col))
                    for col in columns
                ]
...
def _load_sqlite(args: argparse.Namespace) -> None:
    ...
    db_path = _resolve_embedded_db_path(
        SQLiteRuntimeConfig.DATABASE_PATH or ":memory:", "rental_shop.sqlite"
    )
    if str(db_path) == ":memory:":
    ...
    try:
        ...
        for table in _TABLE_ORDER:
            ...
            conn.executemany(
                ...
                [
                    tuple(
                        _embedded_cell_sqlite(table, col, row.get(col))
                        for col in columns
                    )
                    for row in rows
                ],
...
def _load_sqlalchemy_engine(engine_name: str, args: argparse.Namespace) -> None:
    ...
    if engine_name in _MYSQL_FAMILY:
        ...
        database = runtime_cls.DATABASE or __import__("os").environ.get(
            env_key, "rental_shop"
        )
    ...
    connect_args = (
        runtime_cls.connect_args() if hasattr(runtime_cls, "connect_args") else {}
    )
    ...
    if engine_name == "bigquery":
        ...
        if runtime_cls.CREDENTIALS_PATH:
            ...
            bq_client = bigquery.Client(
                project=runtime_cls.PROJECT, location=runtime_cls.LOCATION
            )
            ...
            try:
                if engine_name in _MYSQL_FAMILY:
                    ...
                    _load_bigquery_table(
                        bq_client, runtime_cls.PROJECT, schema, table, frame
                    )
                ...
                raise RuntimeError(
                    f"[load] {engine_name}: failed loading {table}: "
                    f"{_short_load_error_message(exc)}"
                ) from exc
        ...
        if engine_name != "bigquery":
            for table in _TABLE_ORDER:
                for i, fk in enumerate(fk_map.get(table, [])):
                    stmt = translate_alter_fk(
                        engine_name, table, schema, fk, f"{table}_fk_{i}"
                    )
                    if stmt:
...
def _load_postgresql(args: argparse.Namespace) -> None:
    ...
    if args.recreate_schema:
        if args.schema == "public" and not args.allow_public_schema_recreate:
            raise SystemExit(
                "Refusing to recreate schema 'public'. "
                "Use a dedicated schema or pass --allow-public-schema-recreate."
            )
        cur.execute(
    ...
    for name, _, _ in parsed:
        ...
        if pk_cols:
            cols = ", ".join(pk_cols)
            cur.execute(
                f"ALTER TABLE {args.schema}.{name} ADD CONSTRAINT {name}_pkey PRIMARY KEY ({cols})"
            )
...
def _runtime(engine_name: str):
    ...
    if runtime_cls is None:
        raise SystemExit(
            f"Unsupported engine {engine_name!r}. Choose one of: {', '.join(sorted(_ENGINE_CONFIG))}"
        )
    return runtime_cls
...
def _cmd_ping(args: argparse.Namespace) -> None:
    ...
    if args.engine == "databricks":
        ...
        if not url:
            raise SystemExit(
                "Databricks ping requires DATABRICKS_HOST, DATABRICKS_HTTP_PATH, and DATABRICKS_TOKEN"
            )
    else:
        url = runtime_cls.db_url()
    connect_args = (
...
def _trailers_sql(engine: str, schema: str) -> str:
    ...
    return (
        f"SELECT COUNT(*) FROM {item_feature} "
        f"WHERE LOWER(feature_name) = 'trailers'"
    )
...
def _validate_verify_metrics(engine_name: str, counts: dict[str, int], trailers: int) -> None:
    ...
    if missing:
        raise SystemExit(
            f"{engine_name} verify failed: missing or empty tables: {', '.join(missing)}"
        )
    film_count = counts.get("film", 0)
    if film_count != _EXPECTED_FILM_ROWS:
        raise SystemExit(
            f"{engine_name} verify failed: film rows expected "
            f"{_EXPECTED_FILM_ROWS}, got {film_count}"
        )
...
def _verify_duckdb(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        trailers = int(
            con.execute(
                "SELECT COUNT(*) FROM item_feature WHERE LOWER(feature_name) = 'trailers'"
            ).fetchone()[0]
        )
    finally:
        con.close()
...
def _verify_postgresql(args: argparse.Namespace) -> None:
    ...
    connect_args = (
        PostgresRuntimeConfig.connect_args()
        if hasattr(PostgresRuntimeConfig, "connect_args")
        else {}
    )
    sa_engine = create_engine(url, connect_args=connect_args, future=True)
    counts: dict[str, int] = {}
...
def _verify_databricks(args: argparse.Namespace) -> None:
    ...
    if not url:
        raise SystemExit(
            "Databricks verify requires DATABRICKS_HOST, DATABRICKS_HTTP_PATH, and DATABRICKS_TOKEN"
        )
    ...
    try:
        ...
        with sa_engine.connect() as conn:
            ...
            trailers = int(
                conn.execute(
                    text(
                        f"SELECT COUNT(*) FROM {catalog}.{schema}.item_feature "
                        "WHERE LOWER(feature_name) = 'trailers'"
                    )
                ).scalar_one()
            )
    finally:
...
def _load_databricks(args: argparse.Namespace) -> None:
    ...
    schema_name = DatabricksRuntimeConfig.SCHEMA or os.environ.get(
        "DATABRICKS_SCHEMA", "dvdrental_new"
    )
    ...
    try:
        ...
        with sa_engine.begin() as conn:
            ...
            for table in _TABLE_ORDER:
                for i, fk in enumerate(fk_map.get(table, [])):
                    stmt = translate_alter_fk(
                        "databricks", table, target, fk, f"{table}_fk_{i}"
                    )
                    if stmt:
                        conn.execute(text(stmt))
    finally:
...
def main() -> None:
    ...
    if includes and excludes and not args.all:
        parser.error("Cannot combine engine include flags with exclude flags (use --all with excludes, or includes only)")
